<!--
  「自定义玻璃合片单 - 布局编辑」全屏弹窗 —— 旧版 `GlassSheet2PrintManager.openLayoutEditor`
  的那一块（`legacy/js/GlassSheet2.deobfuscated.js` 的 GS:788-798 打开流程 + GS:1371-1852 模板）。

  逆向定稿：`docs/custom-docs-recon/02-glasssheet2.md` §7（§7.1 打开 / §7.2 骨架 / §7.3 控件全清单 /
  §7.4 右侧预览）。

  **它是三套弹窗里的 B**（不是「编辑合片单」那个数据弹窗，也不是「打印设置」）——
  本弹窗改的是**版式**（纸张/标题/边框色/每列的显隐·宽·字号·行高·颜色·顺序），存在
  `glass_sheet2_template_v1` 里，跨会话生效。见报告 §15.6 的三套对照。
...
<template>
  ...
  >
    <div class="gs2-layout-wrap">
      <!-- ─────────────────────────── 左栏：控件（GS:1419-1824） ─────────────────────────── -->
      <div class="gs2-layout-left">
        <!-- 区块一：纸张（GS:1419-1500）。**没有方向下拉、没有常用尺寸按钮** —— 那两样只在打印设置弹窗里 -->
        <div class="gs2-layout-section-title">纸张</div>
        ...
        <div class="gs2-layout-section-title">表格全局</div>
        <n-form label-placement="left" :label-width="80" size="small">
        ...
        <!-- 区块三：各列设置（GS:1600-1824）—— 7 列：显 / 列名 / 宽mm / 字号pt / 行高mm / 颜色 / 排序 -->
        <div class="gs2-layout-section-title">各列设置</div>
        <n-data-table
      ...
      <div ref="rightEl" class="gs2-layout-right">
        <!-- 旧版是 `innerHTML` 直塞（报告 §1 的 `En = ["innerHTML"]`），照抄成 `v-html` -->
        <div class="gs2-layout-canvas-shell" v-html="previewHtml" />
      </div>
    </div>

    <!-- footer 三颗，顺序与语义照旧版（GS:1373-1417）：取消 / 重置默认 / 保存布局(primary) -->
    <template #footer>
      <div class="gs2-layout-footer">
...
import { saveGlassSheet2Config } from '../utils/glasssheet2/storage'
import { paginateWithMeasure } from '../utils/glasssheet2/paginate'
import { createQrSvgProvider, renderPreviewTable } from '../utils/glasssheet2/html'
 ...
 * 用「铺满视口的卡片 + 内容区不滚动」来还原；真正的滚动条在左栏（`.gs2-layout-left`）与右栏
 * （`.gs2-layout-right`）各自身上，与旧版一致。
 *
 * TODO(未确认): 铺满的观感（卡片圆角/外边距/标题与 footer 的高度）没有对着旧版逐像素比过 ——
 * 左栏那 8 条 `gs2-layout-*` CSS 是逐字照抄的，但「卡片外壳」这一层是 Naive 的，不是 el-dialog 的。
 * 真机眼验时看两处：① 整屏无留白；② `.gs2-layout-wrap` 的高度（`calc(100vh - 160px)`）不出现双滚动条。
 */
 ...
 * （读盘的清洗只发生在 `onMounted`，见 `sanitize.ts`；编辑期的草稿不经清洗。）
 */
...
/**
 * 旧版 `L`（GS:162-171）：`clientWidth/Height - 32`（32 = `.gs2-layout-canvas-shell` 的
 ...
 *
 * 空数据 → 占位文案（**不是**空表格）—— 与打印路径的「一个空页」不同（§5.3）。
 */
async function renderPreview(): Promise<void> {
  ...
  const pages = await paginateWithMeasure(rows, cfg, renderOpts) // 旧版 `H(n, o)`
  ...
  previewHtml.value = pages
    .map(
      (page, i) =>
        ...
        (pages.length > 1
          ? '<div class="gs2-layout-page-label">第 ' + (i + 1) + ' / ' + pages.length + ' 页</div>'
          : '') +
        ...
        pd +
        'px;">' +
        '<div class="gs2-prev-table" style="transform-origin:top left;transform:scale(' +
        shrunk +
        ');width:' +
        3.78 * totalMm +
        'px;">' +
        renderPreviewTable(page, cfg, renderOpts) + // 旧版 `S(t, o)`
        '</div>' +
        '</div>' +
        '</div>',
    )
 ...
 *
 * ⚠️ 旧版每次都重新取 `props.getData()`（与 Home 的 `oi` 是**同一份引用**，不是快照）——
...
<style scoped>
/*
  ─────────────────────────── 编辑器专属 CSS（报告 §4.2 的 `gs2-layout-*` 8 条） ───────────────────────────
  旧版这 8 条**不在**组件产出的 HTML 里，而在 scoped 样式表 `legacy/css/Home-97d96482.css`
  （`data-v-c251bbdd`）。新版原样收进本组件的 `<style scoped>`。
*/
.gs2-layout-wrap {
  display: flex;
  gap: 12px;
