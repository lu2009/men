<template>
  <!--
    ...
    **表单本身、初值规则、保存语义一个字节没改**，只是换了容器。
    ...
    旧版这颗按钮要 `userinfo.defaulted === 1`，`sa` 函数内部还**再判一次**。
    ⚠️ **不是只挡扫码账号**：§6.3 那张表里 `0/其它`（普通 PC 账号）**同样看不到**这颗按钮。

    新版没有 `defaulted`，映射到 `role` ⇒ **只有 `admin`**，判断在 `utils/roles.ts` 的
    `canEditProcedures()`（那里有完整理由）。**按钮的显隐由调用方（`Qrscanner.vue`）负责**，
    本组件自己不再判角色 —— 它只管表单与保存。

    ⚠️ 组件**不判**角色这一条是有意的：真正的拦截在后端 `guard.rs`
    （`POST /v1/procedures` 不在 scanner 白名单），前端藏按钮只是别让人白填一遍再吃 403。
    哪天真有第二种入口能进来，拦在**后端**那一层，不是这里。
