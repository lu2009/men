// 自定义玻璃合片单 · 读盘归一化 —— 逐字移植自旧版 `onMounted` 的清洗段（GS:686-768）。
//
// 旧版把整段包在一个 IIFE + `try/catch` 里，**任何异常都落到 `i.value = a()`**（GS:765-767）。
// 新版保持这个语义：`sanitizeConfig()` 内部不抛，出错就返回默认配置。
//
// 字段规则表见逆向报告 §6.4。**两个实打实的旧版缺陷**在那里标了 CONFIRMED：
 ...
 * 即**缺字段补默认、多出的列原样保留**（存了 10 列就得到 10 列，第 9/10 列的
 * `key` 不在默认 8 个之内，渲染时走 `renderCell` 的 `default` 分支 → 空单元格）。
...
export function sanitizeConfig(raw: unknown): GlassSheet2Config {
  ...
  try {
    ...
    return {
      ...
      table: sanitizeTable(w?.table, fallback.table),
...
export function cloneConfig(config: GlassSheet2Config): GlassSheet2Config {
  return JSON.parse(JSON.stringify(config)) as GlassSheet2Config
}
