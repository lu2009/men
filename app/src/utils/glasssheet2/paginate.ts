// 自定义玻璃合片单 · 分页 —— 移植自旧版 `O`（量测，GS:419-518）+ `H`（打包，GS:519-537）。
//
// ⚠️ **这不是收据单那套**（对照表见 §5.2，7 条差异）。最容易抄错的四条：
//   1. 「固定开销」是**硬编码常数** `8 + 10`，**不量测**（收据单是量出来的）；
//   2. **每一页预算完全相同**（本单**没有页脚**；收据单的末页预算不同）；
...
/**
 * 分页预算里的两个**硬编码常数**（GS:525）：`heightMm - 2*paddingMm - 8 - 10`。
 *
 * ⚠️ **一个都别改**（§5.2 明确要求「照抄这两个常数，别"改进"」）。语义是 INTERPRETED：
 *   - `8` 对应 `.gs2-title` 的 `line-height:8mm`（CSS 里正好是 8mm，见 `css.ts`）；
 ...
 * 新版打印链路**建议补同样的 2s 兜底**（`waitForImages(doc, MEASURE_IMAGE_TIMEOUT_MS)`），
 * 那属于组件的有意改进，不改变本层的量测行为。
 */
export const MEASURE_IMAGE_TIMEOUT_MS = 2000

// ------------------------------------------------------------------ //
// A. 打包半（纯函数，H，GS:519-537）
// ------------------------------------------------------------------ //
...
export function paginate(
  ...
  const pages: GlassSheet2Row[][] = []
  let cur: GlassSheet2Row[] = []
  let used = 0
...
/**
 * 量测文档里的样式块（GS:473-474 **逐字**）。
 *
 * ⚠️ 两处必须注意：
 * 1. 选择器是**裸 `th,td`**（不是 `.gs2-table th, td`），因为量测表格没有 `class`；
 ...
 * 组件层不该直接用它，请走 `renderAllPages` / `buildGlassSheet2Html`。
 */
 ...
 * 新版打印链路应传 `MEASURE_IMAGE_TIMEOUT_MS`。
 */
...
/**
 * 量测每行高度（旧版 `O`，GS:419-518）—— **必须真实浏览器**。
 *
 * 流程（§5.1 逐条对照）：
 ...
 * 单元格里的 `<div class="gs2-line">` 都在它**内部**、在文档序里更靠后。
 * （§13 未确认 2 曾担心这点，按实际拼出的 HTML 可以确认无歧义。）
...
export async function measureRowHeights(
  ...
  const table = buildMeasureTable(rows, config, opts)
 ...
 */
export async function paginateWithMeasure(
  rows: GlassSheet2Row[],
  config: GlassSheet2Config,
  opts: RenderOptions = {},
): Promise<GlassSheet2Row[][]> {
  if (!Array.isArray(rows) || rows.length === 0) return [[]] // GS:522
  const heights = await measureRowHeights(rows, config, opts) // GS:526
...
export async function renderAllPages(
  ...
  const pages = await paginateWithMeasure(rows, config, opts)
  return pages.map((page, i) => renderPage(page, i, pages.length, config, opts))
}
 ...
 */
export async function buildGlassSheet2Html(
  rows: GlassSheet2Row[],
  config: GlassSheet2Config,
  opts: RenderOptions = {},
): Promise<string> {
  const pages = await renderAllPages(rows, config, opts)
  return buildRootHtml(pages, css(config))
}

/**
 * 构建**完整文档**（旧版 `J`，GS:673-681）—— 带 `<!DOCTYPE html>` 与 `<title>`，
 * 给 `printDirect` 的隐藏 iframe / Electron 静默打印用。
 */
export async function buildGlassSheet2Document(
  rows: GlassSheet2Row[],
  config: GlassSheet2Config,
  opts: RenderOptions = {},
): Promise<string> {
  return buildDocumentHtml(await buildGlassSheet2Html(rows, config, opts))
}
