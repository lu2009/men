// 自定义玻璃合片单 · 打印链路 —— 移植自旧版组件 expose 的 `printDirect`（GS:799-848）。
//
// ⚠️ 本文件是**组件层新增**的（同 `qr.ts`：没动核心逻辑层的任何既有文件）。
//    放在这里是因为它整段都是「造 HTML → 隐藏 iframe → 打印」的纯逻辑，与 Vue 无关。
//
// 旧版有**三条**打印路径（报告 §10.1），新版只做第一条：
//   · `printDirect()`          —— 本文件。**无参**，自己重建 HTML 字符串（不是 clone 预览 DOM）。
//                                 Home 的「手动打印」按钮走它（§11.4 key=22）。
//   · `printSilent()`          —— Electron IPC 静默打印（GS:849-889）。**新版没有 Electron**，
//                                 载体不存在 ⇒ 不做（与收据单2 同一处理，见 `receipt2/print.ts` 头注）。
...
import { MEASURE_IMAGE_TIMEOUT_MS, buildGlassSheet2Document, waitForImages } from './paginate'
 ...
 *
 * 时序（GS:799-848，逐条）：
 *   1. 构建**完整文档 HTML**（`<!DOCTYPE html>…<title>自定义玻璃合片单</title>…`，见 `paginate.ts`）；
 ...
 */
export async function printGlassSheet2Direct(
  rows: GlassSheet2Row[],
  config: GlassSheet2Config,
  opts: RenderOptions = {},
): Promise<void> {
  const html = await buildGlassSheet2Document(rows, config, opts) // 旧版 `J()`，GS:806
