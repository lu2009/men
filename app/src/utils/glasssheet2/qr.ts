// 自定义玻璃合片单 · 二维码编码器 —— 把 `qrcode-generator` 接到核心层的 `QrEncoder` 上。
//
// ⚠️ 本文件是**组件层新增**的（不属于 `utils/glasssheet2/` 原有的核心逻辑层，
//    没有动 `types.ts` / `html.ts` / `paginate.ts` 等任何一个既有文件，也没往 `index.ts` 里加导出）。
//
// 旧版（GS:203 / GS:245-260）用的是 vendor chunk 里内嵌的 `@zxing/library` 系编码器：
//   `k = new QRCodeClass()`
...
function svgToParts(svgText: string): QrSvg | null {
  ...
  if (!svg) return null
  return {
    // 空串交给核心层的 `|| "0 0 180 180"` 回退（与旧版同一条路径）
...
/**
 * 造一个 `QrEncoder`（核心层 `createQrSvgProvider` 的入参）。
 *
 * 旧版是模块级单例 `k = new y()`；`qrcode-generator` 的用法本来就「一次文本一个实例」，
 * 所以这里每次调用新建，缓存由核心层的 provider 负责（`text + "::m1"` 那张表）。
 *
 * 返回 `null` 的两条分支都照旧版：
 *   · 空文本（核心层 provider 已经先挡了一道，这里再挡一次防直调）；
