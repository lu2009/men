...
| 截止日期 | `截止日期` | Date | = 日期 + **生产天数**（~~+ 1~~，2026-09-19 更正，见下） | 交付截止 |
...
> ⚠️ **2026-09-19 更正（这一格当年写错了）**：本表原来写「= 日期 + 生产天数 **+ 1**」，
> **没有代码出处**，而且与旧版源码不符 —— `Hui.formatted.js:8843`（就在「3.保存回执单」里）写的是
> `new Date(日期.getTime() + 24 * 生产天数 * 60 * 60 * 1e3)`，即 **日期 + 生产天数**，没有 +1。
> 这个多出来的「+1」后来被 `docs/home-audit/00-summary.md` 拿去当提论证过一次（循环论证），
> 还进了后端 SQL（`orders/service.rs`）与 `Hui.vue` 的推导式 —— 2026-09-19 按用户拍板全部改回 **+0**，
> 详细取证见 `docs/2026-09-19-hui-shell-audit.md` §6。
...
- 优先级（**旧版原义，2026-09-16 更正**）：`自定义方数 > -1` 时它只是**每樘下限**，参与
  `max(单樘面积, 自定义方数)`；否则 `max(单樘面积, 后端 square 表按 formulaid 的最低平方数)`。
  ⇒ 旧版里自定义方数**只会把面积抬上去、不会压下来**，× 数量恒在外层。
- 组件 C 的 square 配置支持按「扇数」索引（含 `a-b` 按有无亮窗取值的字符串形态）。
- 落地：`平方数 = 单樘方数 × 数量`。
- ⚠️ **新版有意偏离**（老板 2026-09-16 要求「业务上真要手动改小」）：`自定义方数 > -1` 时改为**覆盖**，
  填多少算多少（可低于面积、可低于最低平方数）；留空即回自动。见 `Hui.vue` 的 `computeSquare`。
