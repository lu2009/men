...
> ⚠️ **本段 2026-09-19 重写过。** 原摘要点名的「9 条工具按钮」里多数**早已补做**，
> 摘要没跟上（这类「表里写的和文档别处写的互相矛盾」本仓库栽过三次）。
> 现按**逐条复核后的实测值**重写：**82 条带判定（20 ✅ / 47 ⚠️ / 14 ❌ / 1 ❓）**，
> 数字由 `node docs/home-audit/recount-status.mjs` 重算（口径见该文件头；Y/Z 两态子表 19 条不计入）。

- 工具栏：旧版**实际 10 颗**（左 9 + 右 1）。新版**只剩 1 颗 ❌ —— 「视频」**
  （旧版 `Rt` 打开 3 条抖音链接的教程抽屉；`grep 视频` 在 `Home.vue`/`AppHeader.vue` 零命中）。
  原摘要点名的「导出 Excel 生产单 / 列宽设置」**不是工具栏按钮**（前者在 `ic=2` 打印预览弹窗，
  后者在君豪汇总订单弹窗内，见 A 段坐标更正）；**「查询更多」「合并订单」「扫码生产」都已补**
  （A6 / A11 / A15，各带残余偏离）。
- FinanceDrawer：三 tab、四弹窗、默认项、红冲色、Tab 懒加载 —— 结构 ✅。
  原摘要说「§5.4 校验几乎全缺 ❌」**已不成立**：②–⑦ 六条校验**已在服务端实现**
  （`finance/service.rs`），前端 `form.validate` 仍缺 ⇒ 改为 ⚠️（B22 / B26 / B27）。
  收款方式的**自定义新增与右键删除**仍未做。
- 经营看板：11 块面板与筛选结构齐全；差异集中在 DataV 组件降级、`流程状态` 左右顺序、
  `最新订单` 列集、数据源按角色过滤，以及 **「本周」起点（周一 vs 旧版周日）**（C3，有意偏离）。
- `syncNoticeBoard` ❌ 完全未做（旧版挂在全局路由守卫，**不在 Home 内**）。
- `Yt`（工厂/终端）影响 12 处 UI、`z`（手机端）影响 7 处 —— 逐条列在 §E；
  **这两张子表 19 条不计入上面的 82**（新版有意只做工厂视图，把它们算成「未做」会失真）。
...
| A6 | **查询更多** | `:11268–11272` `el-button.custom-search-btn` 文案 `s(1355)=" 查询更多 "`；handler `ms` @ `:11029`（开 `.查询订单` 弹窗 + 拉 `getClientsInfo`）；确认 `ys` @ `:11052`（GET `getMoreTableDate&param2=ds&param3=客户&param4=地址&param5=start&param6=end`，按 `打单人` 二次过滤，与 `_l` 去重合并，再把搜索框写成「客户 地址」，`Yt&&Ht` 时 `ea(60,{start,end})` 重拉财务摘要，并刷新看板）；「确认统计」`vs` @ `:11122` | **无** | ⚠️ 偏离（2026-09-19 复核） | 「查询更多」**已做**：按钮 `Home.vue:12` + 「查询订单」500px 弹窗 `:295-353`（字段顺序与旧版逐条对齐）+ `openQuery()`/`submitQuery()`（`:1287-1374`）。**残余偏离**：①「确认统计」分支有意不做（旧版该分支不可达，`:1214` 注释）；②「只含生产单」旧版在查询路径上是**死 checkbox**，新版按标签本义接上了 `progressMatch("已打生产单")`（`:1329-1332`，注释自认）。 |
...
| A11 | 合并订单 | `:11289–11293`：**仅 `Fl.length>1` 时渲染**，class `custom-combine-btn(#67c23a)`，文案 `" 合并订单 ("+N+") "`；handler `Ii` @ `:9190` | **无** | ⚠️ 偏离（2026-09-19 复核） | 「合并订单」前后端都做了（见 `02-actions.md` J6）⇒ 原判定「新版完全没有」已失效。**残余**：① 旧版还会把 `截止日期` 取最早，我们没做；② 成功文案不同。⚠️ **顺带更正**：**旧版取的也是「最早/最小」那一单**（`Home.formatted.js:9196` 的 sort 比较器是 `parseInt(回执单号)` **升序**，取 `l[0]`，与弹窗文案「以最早的回执单号为准」自洽）—— 原审计写的「最大者为主」是错的，后端口径本来就是对的。 |
...
| A15 | **扫码生产** | **不在 Home 内**。`index-c3b16e3f.js@29940`：壳抽屉菜单项 `📱 扫码生产`（`el-menu-item`，`onClick:O()`，受 `we && !ve` 门控，`ve`=手机端）；路由 `/Qrscanner` @ `index@73213`；且是**非终端账号登录后的默认落地页**（`index@81951` `n(1===t ? "/hui" : "/Qrscanner")`） | **无**（`app/src/router/index.ts` 只有 `/ , /login, /formulas, /clients, /hui, /receipt-view/:receiptNo, /receipt-share`；`App.vue` 无壳导航） | ⚠️ 偏离（2026-09-19 复核） | 「扫码生产」已做：路由 `/qrscanner`（`app/src/router/index.ts:28`）+ 壳菜单项（`AppHeader.vue:92`）+ 整页 `Qrscanner.vue` + `roles.ts:188` 的落地规则 ⇒ 原判定「完全缺席」已失效。**残余**：落地只对 `role="scanner"` 生效（旧版是 `defaulted !== 1` 全落，`roles.ts:183` 自认有意偏离）；旧版「手机端隐藏」那条门控无对应物。 |
...
| B2 | 三 tab 名称 | `:1505–1510` `name:"order"` label `"📄 本单财务"` + `el-tag "单"`；`:1686–1691` `name:"payment"` `"💰 客户收款"`（tag）；`:1838–1843` `name:"statement"` `"📊 客户对账"`（tag） | `FinanceDrawer.vue:6, 94, 149` | ⚠️ 偏离（2026-09-19 复核） | 三个 tab **只有纯文案**，旧版每个 tab 后面还挂一颗 `el-tag`（`Home.formatted.js:1505-1510` 是「单」、`:1686-1691`/`:1838-1843` 是 `To(389)`="客户级"），三颗都没复刻。tab 文案本身逐字一致。 |
...
| B26 | §5.4 **订单抹零**（`Ve` @ `:1327–1350`） | ① `null/0` → warning「请输入调整金额（正数=减免，负数=冲销）」`:1320`；② `n>0 && n>未收+0.005` → error「抹零金额（X）不能超过订单未收金额（Y）」`:1321`；③ `n>0 && n>客户余额+0.005` → error「抹零金额（X）超过客户当前余额（Y），请检查是否已有客户级抹零」`:1323`；④ `n<0 && \|n\|>订单调整金额+0.005` → error「冲销金额绝对值（X）不能超过已有订单调整合计（Y）」`:1327` | `FinanceDrawer.vue:582–603` | ⚠️ 偏离（2026-09-19 复核） | 服务端已兜住（`backend/src/modules/finance/service.rs:413-430` 的 `add_order_adjustment`：超本单未收 / 超客户余额 / 冲销超订单调整合计）。**残余**：`FinanceDrawer.vue:776-778` 仍是无提示的 `if (amount == null) return`；旧版第 ① 条「请输入调整金额」前后端都没有（0 元抹零会被真的写库）。 |
| B27 | §5.4 **客户抹零**（`me` @ `:1349–1372`） | ① `null/0` → warning「请输入调整金额（正数=减免，负数=冲销）」；② `n>0 && n>客户余额+0.005` → error「抹零金额（X）不能超过客户余额（Y）」`:1356`；③ `n<0 && \|n\|>客户调整合计+0.005` → error「冲销金额绝对值（X）不能超过客户调整合计（Y）」`:1360` | `FinanceDrawer.vue:616–636` | ⚠️ 偏离（2026-09-19 复核） | 同上：`backend/src/modules/finance/service.rs:500-514` 的 `add_customer_adjustment` 已做超客户余额 / 冲销超客户调整合计；① 仍缺。 |
...
| C3 | 筛选：日期区间 | `:423–424` `el-radio-group` 6 项 `全部/今天/本周/本月/上月/90天`（值 `wl(610/537/566/599/508/613)`）；dayjs 计算 @ `:214–246`：今天 `startOf('day')`、本周 `startOf('week')`、本月 `startOf('month')`、上月 `subtract(1,'month').startOf/endOf('month')`、90天 `subtract(90,'day')`；过滤是**字符串比较** `t["日期"] >= d && t["日期"] <= m`（`:248–252`） | `DashboardBigScreen.vue:14–23, 179–200` | ⚠️ 偏离（2026-09-19 复核） | **「本周」起点不同**：旧版 dayjs 用 en 默认 locale（`weekStart=0`=**周日**），我们手算**周一**（`DashboardBigScreen.vue:187-192`），且上界只到 today、本月/90 天还没有旧版那个 `<= 今天` 的上界。**仓库自己已认定为有意偏离**（`app/src/utils/productionStats.ts:24-33`）。影响：周三看看板，旧版含上周日那天的单、新版不含。⚠️ 与 `docs/2026-09-19-progress-dashboard.md` §15.2 第 4 条那个**待拍板**是同一件事。 |
...
| C5 | 数据来源 | `:12191` `<DashboardBigScreen modelValue=ks tableData=_l.value onCustomQuery=ms />` —— **`_l` 是 `getTableData` 的原样全量列表，未按角色过滤** | `Home.vue:188` `:orders="dashboardOrders"`（模板绑定；模板自 2026-09-20 起未改）；`dashboardOrders` 的**声明** @ **`app/src/composables/home/useHomeData.ts`**（2026-09-20 Task 5 从 `Home.vue` 搬出），**按 `auth.user.role!=='admin'` 过滤成「只看自己打的单」** | ⚠️ 偏离 | 旧版看板看**全店**数据；新版非管理员只看自己的。⚠️ 本行两处**行号**在本笔之前即已过期（旧号 `655–659` 从来就不是 `dashboardOrders`）—— 全仓 `Home.vue:<行号>` 指针的一次性对账见 Ruling 173（Task 15 做），本笔只修**名字级**那半句。 |
...
| C8 | 中列：每日订单趋势 | `:521–529` 标题 `wl(509)="每日订单趋势"` + `div.chart-container ref=trendChartRef`；echarts 折线 `:290` 起；数据 `b` computed `:305–320`：按 `日期` 聚合 **`count` 与 `amount`** 两个字段，按 date 升序 | `DashboardBigScreen.vue:90–101, 262–297` | ✅ 已做（2026-09-19 复核） | 旧版的折线**只有一个 series、`data` 只取 `count`**（`Home.formatted.js:311-312`）—— 原说明的「聚合 count+amount、双系列」不成立（`amount` 旧版只算不画）。剩余差异（手写 `<polyline>` vs echarts）与 C6/C9 同类，而那两行都判 ✅。 |
...
| C12 | `customQuery` 事件 | `:161` `emits:["update:modelValue","customQuery"]`；Home `:12191` 绑 `onCustomQuery:ms` | — | ✅ 已做（2026-09-19 复核） | **死代码，不做等于做对**：`customQuery` 在 `DashboardBigScreen` 全组件只出现在 `emits` 声明里、**从未 `emit`**（旧版 Home `:12191` 绑的 `onCustomQuery` 永远收不到）。原判定 ❌ 与本行自己的说明（「新版不复刻属**正确**」）**自相矛盾**，已按本文对死码的既有口径（A10/B2/F8）改正。 |
...
| Y5 | **财务摘要拉取**：仅 `Yt && Ht` | `:7904` | 新版无条件（见 B33） | ✅ 已做（2026-09-19 复核） |
...
| Y9 | **查询更多弹窗的「客户」字段**：仅 `Yt` 渲染（终端用手填 `id` 反查） | `:12135–12146`；`ys` 里 `!Yt` 时用 `nl.find(t=>t.id===jt.value)` 回填客户名 | 新版无此弹窗（见 A6） | ✅ 已做（2026-09-19 复核） |
| Y10 | **打印抽屉的两颗回执单动作**：`直接打印回执单`/`手动打印回执单` 仅 `Yt`（`:11876–11897`） | 同上 | `PrintDrawer.vue:50–67` 只有「查看回执单」「回执单-其它」 | ⚠️ 偏离（2026-09-19 复核） |
...
| Z2 | 「复制成图片 / 微信分享(手机)」二选一（`:11783/11787`） | 同上 | 新版 `ReceiptOtherDialog` 未做此二选一 | ❌ 未做（2026-09-19 复核） |
| Z3 | 「复制玻璃单 / 微信分享(手机)」二选一（`:11799/11803`） | 同上 | 同上 | ❌ 未做（2026-09-19 复核） |
| Z4 | 「导出玻璃订单」按钮 `lo && !z`（仅桌面）（`:11811`） | 同上 | 无此按钮 | ❌ |
| Z5 | 打印抽屉：`直接打印回执单/手动打印回执单` `z.value ? 无 : 按钮`（`:11884`） | `:11884` | 无 | ⚠️ 偏离（2026-09-19 复核） |
...
| F9 | 行状态色 | `.date-audit{background:#ffb6c1;color:#721c24;padding:4px 8px;border-radius:4px;font-weight:700}`、`.date-warning{background:#fff3cd;color:#856404;…}`、`.date-danger{background:#f8d7da;…}`、`.paid-customer{background:#90ee90;…}`、`.loaded-row{#dbdbd8}`、`.duplicate-order-row{#ffe4ec/`#ffd6e4}`（CSS 文件） | `Home.vue` 行状态色一节 + `<style>` | ✅ 已做（2026-09-18） | 六条里做了五条，都带 `color`/`padding`/`border-radius`。⭐ 剩的那条 **`.date-danger` 是死类**：`grep -c date-danger legacy/js/Home.formatted.js` = **0** —— CSS 有规则，但**旧版 JS 从不加这个类**（`Ls` 只产出 `date-audit`/`date-warning`）。有意不做，理由与 `.paid-row` 同类。 |
...
| F12 | 分页 | `:11608–11611` `el-pagination layout:"total,sizes,prev, pager, next"`、`page-sizes Xl=[10,20,50,100,200]`（`s(531)` 处 doc 已记）；默认 `Zl=50` | `Home.vue:77–84` `:page-sizes="[10,20,50,100,200]"`、默认 `pageSize=50`（`:375`） | ✅ 已做（2026-09-19 复核） | 旧版 `layout:"total,sizes,prev, pager, next"` **本来就没有跳页框** ⇒ 原说明的「新版多了 quick-jumper」已失效；`page-sizes` 五档与默认 50 都对（`Home.vue:130-150`、`:375`），`item-count` 等效。 |
| F13 | 表格高度 | `:11300` `height:"calc(105vh - 280px)"`（硬编码在 el-table 上） | `Home.vue:1119` `calc(100vh - 300px)` | ⚠️ 偏离 | 数值与参照系都不同（105vh vs 100vh）。 |
| F14 | 表格 `filter-method` / 列头 `filters` | `:11300` `"filter-method":ga`；各列 `filters` @ `:11437/11445/11468/11516/11524/11532/11583/11591` | `Home.vue:1019–1117` 无 `filters`/`filter-method` | ✅ 已做（2026-09-19 复核） | 列头筛选整块**已实现**：状态机 `Home.vue:735-870`、逐列 `filterOptions`/`filter`/`filterOptionValues`（`:3056` 起十余处）、`@update:filters`（`:124`）⇒ 原判定「无 filters/filter-method」已失效（口径差异见 `01-table.md` C16）。 |
...
| G3 | 二维码产出 | Home 打印链路里拉的 `getimage&param2=qrcode`（收款二维码，`:9383–9395`，缓存到 `M("qrcode","qrcode",blob)`）；`docs/...-home-analysis.md` §12.4 已确认「全库 type 仅 `qrcode` 一种」 | 新版未见对应实现 | ⚠️ 偏离（2026-09-19 复核） | 打印链路的收款码**已实现**（`app/src/composables/useOrderPrint.ts:17-18/97` 的 `idbGetImage("qrcode")`、`Hui.vue:469` 的「收款码设置」上传口）⇒ 原判定「新版未见对应实现」已失效。**来源不同**：本机 IndexedDB 用户上传 vs 旧版服务端 `getimage&param2=qrcode`。另 `ReceiptCard.vue:10-14` 仍**有意**去掉支付二维码。 |
