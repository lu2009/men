...
| J9 | 批量：表头「付款状态」popover（已付/未付/部分付/全部显示）、「生产进度」popover（4 固定项 + 自定义 + 显示全部） | `dr(779)`=付款状态 @`479345`；`dr(1216)`=生产进度 @`482765`；`Bo=dr(1187)/dr(1309)/"已订玻璃"/dr(1202)` | `Home.vue:1075` / `1101`（`popoverTitle`） | ✅ 已做（2026-09-19 复核） | 三条偏离全改正：`PAYMENT_OPTIONS`/`PROGRESS_OPTIONS` 顺序对（`Home.vue:497-500`）、自定义项那一组补上（`:3225-3236`，`dividerBefore` 只挂第一个自定义项、没有自定义项就不插分隔线）、「显示全部」**收尾**；选项文案逐字对（`dr(1187)`/`dr(1309)`/「已订玻璃」/`dr(1202)`）。 |
