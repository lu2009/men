...
// ───────────────────────────────────────────────────────────────────────────
// 切源码的小工具（按花括号配平切，**不按行号 / 不截断** —— 本项目为此栽过两次）
// ───────────────────────────────────────────────────────────────────────────
...
const HUI = readFileSync(`${ROOT}/legacy/js/Hui.formatted.js`, 'utf8')
const HOME = readFileSync(`${ROOT}/legacy/js/Home.formatted.js`, 'utf8')

/** 跑 Hui 自带的解码器（数组函数 + 解码函数 + **轮转 IIFE**，缺轮转索引整体错位）。 */
function huiDecoder() {
  // 从函数头一路切到配对的 `}` —— **要含函数头**，只取 `{…}` 就变成了一个块语句，
  // `_0x250a` 压根不会定义（本台子第二版栽在这）。
  const cutFn = (header) => {
  ...
  // ⚠️ 整段 `!function(e,t){…}(实参);` 一起切，**不能只取 `{…}` 再自己拼 `()`** ——
  // 这是个带形参的 IIFE，漏掉实参轮转就不跑，索引整体错位（本台子第一版就栽在这）。
 ...
 *   （Hui 这段是 `r`，Home 这段是 `V`，两处都指向各自的表，且 `V=n` 这种二次别名
 *   要顺着作用域往上找才解得开）—— 写死名字的下场是 `ReferenceError: V is not defined`。
 *
 * 判据是**可证伪的**：这段代码里出现 `名字(数字)` 的每一处，正确的那张表必须**全部**解得开
 * （解不开返回 `undefined`，不是字符串）。命中数必须**恰好 1** —— 命中 0 说明切错段，
 * 命中 ≥2 说明判据不够严，两种都不许往下走。
 */
function resolveDecoders(code, candidates, label) {
  const calls = new Map()
...
function deobf(code, table, label) {
