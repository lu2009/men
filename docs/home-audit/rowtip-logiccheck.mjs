 ...
 * 与新版表示逐条比。
 *
 * 另钉住一条容易看走眼的：旧版那个「tooltip 跟着鼠标走」的 `Va`（`:7983`）
 * **全文件零调用** —— 写了没接上。所以旧版实际是「位置只在进入单元格那一刻取一次」。
 *
 * ⚠️ **新版接上了它**（`Home.vue` 跟随鼠标、位置直接写 DOM 不经过响应式）——
 * 那是有意的，理由见该文件 `showRowTip` 的注释（旧版实测表现为「挂在那儿不动 + 收不掉」）。
 * 这条断言只保证「旧版确实没接上」这个**事实**仍然成立，不代表新版要照做。
 *
