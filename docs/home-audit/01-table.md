...
| B8 | 取值：拆分成多单号 | `:7699-7702` `To(e)` → `Uo(单号集)`，`:7694-7697` `Uo = s.trim().split("_").map(trim).filter(Boolean)` | `Home.vue:1045-1046` `set.split(/\s+/)` | ⚠️ 偏离 | **分隔符不同**：旧版按 **下划线 `_`** 拆，新版按**空白**拆。`backend/migrations/0018_home_order_head_fields.sql:4` 写的是「多个单号的空格串」，与旧版 `split("_")` 矛盾——两边至少有一份是错的，需要按真实数据定夺。（新版目前没有任何写入方，`order_no_set` 只在创建/更新时由入参带入，见 `backend/src/modules/orders/service.rs:319/372`，所以实际影响取决于用户手填格式。） |
| B9 | 单元格显示：只显示第一段 | `:7702` `l[0] \|\| ""`；`:11368-11373` hover popover 列出全部 | `Home.vue:1047-1050` | ✅ 已做 | 结构一致（首段 + hover 看全）。 |
| B10 | 列头「查单号」popover | `:11366-11384`：表头内嵌 `el-popover` + 输入框（placeholder 待查）+ 清除/确认，`Yo` 按 `po` 过滤首段 | 无 | ❌ 未做 | 新版单号集列没有列头筛选，`To` 里那段「按输入过滤首段」的语义（`:7700-7701`）也就没落点。 |
| B11 | 单号集列头「清除」按钮 | `:11367-11371`：`po` 非空时显示 `text` 按钮「清除」 | 无 | ❌ 未做 | 同上，随 B10 一起缺。 |
...
| B25 | 单元格形态 | `:11440-11444` / `:11527-11531`：**常驻** `el-input type="textarea" autosize{1,3}` `class="input-style"`，`onFocus` 才置 `za=row` 进编辑态 | `Home.vue:954-973` `renderEditable`：非编辑态是纯 `div.editable-cell` | ⚠️ 偏离 | 旧版格子永远是个无边框 textarea（CSS `el-input__inner{border:none;padding:2px 5px}`，见 §11.2），聚焦才变可编辑；新版非编辑态是 div、点击才换输入框。结果接近，观感不同。 |
...
| B27 | 列头 popover 触发器 | `:11534-11546`：表头 = 文字「打单操作」+ `text` 按钮「 生产进度 」+ 当前值 `(值)` | `Home.vue:1075` `popoverTitle('打单操作','生产进度',...)` | ✅ 已做 | 结构对（列头文字 + `生产进度` 按钮 + 列表）。少一项：旧版按钮后带 `(当前选中值)`，新版不显示当前值（见 C9）。 |
...
| B31 | 单元格底色（`la`） | `:7940-7942` `la(e)`：含「收据单」→`#90EE90`、含「标签」→`#FFC0CB`、含「玻璃订单」→`#87CEEB`、含「生产单」→`#FFFF99`、含「自助下单」→`#FFA500`（**按此优先级短路**）。**调用点只有 2 处**：`:11589`（业务员列）与 `:11597`（打单人列） | `Home.vue:931-938` `statusBg` → 用在**打单操作**格子 `Home.vue:1079` | ⚠️ 偏离 | ⚠️ **同时更正既有文档**：`docs/2026-09-17-home-analysis.md` §3 写「「打单操作」单元格底色（`la`）」—— **错的**。`la()` 全组件只在业务员/打单人两列的**非管理员分支**被调用；打单操作格子的外层 div（`:11571-11582`）只有 `style:{cursor:"pointer"}`，**既无 class 也无背景色**，`legacy/css/Home-97d96482.css` 里也 `grep` 不到这五个色值（`FFFF99` 等命中数 = 0）。所以：新版**把色映射用错了列**（打单操作格凭空多了底色），而**真正该有的业务员/打单人非管理员底色反而没做**（见 B43/B44）。另：`la` 返回整串内联样式（含 `padding:4px 8px;border-radius:4px;font-weight:bold`），新版只取背景色。 |
...
| B40 | 未付列头 popover | `:11475-11508`：表头内 `el-popover`（width 220），reference 是 `text` 按钮「 付款状态 」+`(当前值)`；option = 已付 / 未付 / 部分付 / 全部显示 | `Home.vue:1101` `popoverTitle('未付','付款状态',PAYMENT_OPTIONS,...)` | ✅ 已做 | 机制、文案、拦截语义都对（详见 C6–C8）。 |
...
| # | 项 | 旧版依据（行号） | 新版落点（文件:行） | 判定 | 说明 |
|---|---|---|---|---|---|
| B42 | 订单备注列 | `:11516-11523`：`{label:"订单备注",prop:"订单备注","min-width":220}`，常驻 `el-input textarea` | `Home.vue:1109-1114` | ⚠️ 偏离 | 宽度对、可编辑对；形态差异同 B25（常驻 textarea vs 点击进编辑），且 `filters` 未做。 |
...
| C7 | 选项**顺序** | `:11482-11507` 渲染顺序：已付 → 未付 → 部分付 → 全部显示 | `Home.vue:236` `['全部显示','已付','未付','部分付']` | ⚠️ 偏离 | 旧版「全部显示」在**最后**，新版放**最前**。 |
| C8 | 触发器显示当前值 | `:11479-11481` reference = 按钮「 付款状态 」+ 选中时 `(值)` | `Home.vue:976-1014` `popoverTitle`：只显示 `label + ▾`，值只在 popover 内高亮 | ⚠️ 偏离 | 旧版列头能直接看到当前筛选值 `付款状态 (未付)`，新版看不到。 |
...
| C13 | 选项顺序 | `:11547-11573`：4 固定项 → 分隔线(`:11574`, `La.length` 时) → 自定义项 → 「显示全部」(`:11575-11581`) | `Home.vue:237` `['显示全部','已打生产单','未打生产单','已订玻璃','未订玻璃']` | ⚠️ 偏离 | 旧版「显示全部」在**最后**（且清理走 `Po`，`:7680-7681` 还会重置页码），新版放**最前**；分隔线也没有。 |
| C14 | 触发器显示当前值 | `:11549-11553` reference：按钮「 生产进度 」+`(值)` | `Home.vue:1075` | ⚠️ 偏离 | 同 C8。 |
