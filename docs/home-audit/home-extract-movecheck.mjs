 ...
 *
 * ⚠️⚠️ **能力边界之二 ── 它抓不到「登记本身写错」**（2026-09-20 实测撞出来的）：
 *   本脚本证明的是「搬迁**符合登记过的改写**」，**不是**「登记过的改写**是对的**」。
 *   某个 block 的 `rewrites` 写错时，REF 侧套的是**同一份错规则** ⇒ **两边同错** ⇒
 *   归一化后逐字一致，**照样绿**。
 *   实证：B4（`useHomeOrderNo`）第一版把该块**自己拥有**的三个 ref（`orderNoInput` /
 *   `orderNoRestoring` / `orderNoPopShow`）也当成注入项、体里写成 `deps.orderNoInput.value`
 *   ⇒ **8 处 `TS2339`**，而本脚本**全绿**（同一条错规则也套在 REF 侧了）。
 *   ⇒ 抓这一类的是 **`vue-tsc` / `npm run build`**，不是本脚本。
 *   ⇒ **结论：两者必须成对跑，缺一不可** ——
 *     本脚本管「搬的时候有没有**偷偷改**」；`vue-tsc` 管「搬完的**名字与形状对不对**」。
 *     只跑一个都会漏掉另一半（另见上面 `sliceFn` 那条：那是同一类盲区的切片版本）。
 *
...
const BLOCKS = [
   ...
   *    ⇒ **守卫管「有没有改」，不管「改得对不对」**；后者只有 `vue-tsc` 与 37 个台子抓得到。
   *
