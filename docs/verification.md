...
〔**2026-09-20 Task 8 更正：这两个数原来是 `29` / `36`。** 总数 **37** 是**静态数出来的**，
不是按增量推的 —— 复刻 `run-all.mjs` 的 `collect()` 逐目录数实际存在的台子文件：
`docs/` **14** + `docs/home-audit/` **19** + `docs/legacy-finance/0N-*.mjs` **5** − `MANUAL` 1 = **37**；
`30` = 37 − 7，其中 ⏭ 的 7 = `verify.mjs` 的 `NEEDS_LEGACY_SRC` **6** + `NEEDS_SEED` **1**。
⚠️ **下面那段按时间顺序只记到接入 `docs/progress-extract-movecheck.mjs` 之前的一步（总数 36）**
—— 它是有日期的历史记录，**故意不改**；现状以这张表为准。〕
〔⚠️ 另：这张表的 ✅ **不是**一次实测转录，是「收集集合 − 未运行」的静态口径；
今天**没有**在 CI 上真跑过（用户下令「只干活、不跑验证」）。〕
〔⚠️ 本节开头那句「那 37 个台子打的全是真后端 + 真 PG」现在**不成立** ——
37 里含 3 个 `*-movecheck.mjs`（只读 git + 文件，不碰后端）。原话未改，标记在此。〕
