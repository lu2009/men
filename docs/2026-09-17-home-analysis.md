...
`回执单号`(主键/排序) · `单号` · `单号集` · `日期` · `截止日期` · `客户` · `客户编号` · `电话`/`地址` · `业务员` · `打单人` · `打单操作` · `品牌` · `门数` · `总价` · `定金` · `锁向` · `加价项目` · `安装地址` · `订单备注` · `已分配金额`/`订单调整金额`/`未收金额`(新财务) · `类型`(流水过滤)。
...
**订单头缺**：单号、单号集、打单人、打单操作(状态机)、锁向、加价项目、独立「截止日期」字段。
