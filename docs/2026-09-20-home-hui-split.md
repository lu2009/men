...
| **B1** | `composables/home/useHomeData.ts` | **实测**：538–544 + 546–560 + 1540–1548<br>（方案原写 `538–576` —— 多算了 **`onMounted`(562–571) 与 `onLogout`(573–576)**，这两段**故意留页面**：`onMounted` 的 `570` 写的是 B6 的 `homeFormulas`，跟着搬会让 B1 **反向依赖 B6**。见 §8.2 #10①） | **实测 注入 2 / 输出 6**：<br>注入 `message`（`load` 失败提示）· `auth`（看板按角色过滤）；<br>输出 `loading` · `rawOrders` · `financeSummary` · `load` · `dashboardShow` · `dashboardOrders`。<br>⚠️ 方案原写「4 项」，数的是**输出里那 4 个声明** —— 「注入面」与「输出面」是两回事（B10 那格同病，见 §8.2 #8①） | 干净的根块（数据/加载 + 看板）。<br>✅ **2026-09-20 实做完毕**：6 个声明逐字搬迁（守卫 `home-extract-movecheck.mjs` 的第三条），`npm run verify` **35 ✅ / 0 ⏭ / 0 ❌**，`vue-tsc` 零错。**搬走 32 行**（含 1 行空行，非空 **31**）= 段① `514–536`（23 行）+ 段② `1368–1376`（9 行，含两段段头注释）；新文件 **65 行**。`Home.vue`：`git diff --numstat` **+20 / −28**，净 **−8**（3122 → 3114）。<br>⚠️ 同笔动了 **1 个类型导入**：`OrderFinance` 随 `financeSummary` 一起失去用处 ⇒ `vue-tsc --noEmit` **TS6196**（`npm run build` 红 ⇒ `verify` 整体退 1）⇒ 删掉 + 4 行说明（与 B10 那个 `DataTableRowData` 同类，见 §8.2 #10③）。<br>⚠️ 同笔改了 **`docs/roles-admin-logiccheck.mjs`**（**这是第一个被搬迁绊红的台子**，而且「扩源」不够 —— 见 §8.2 #10④）。 |
...
| 9 | ✅ **【已修，2026-09-20 Task 3.5】守卫自身的洞：`sliceFn` 对「声明跨行」的形状切不完整。** 原报告（保留以便追溯）：`sliceFn` 找函数体 `{` 的循环末尾有一句 `if (c === '\n') break` —— 参数表一旦换行，函数体 `{` 就在那个换行之后，永远找不到 ⇒ 退化成「按行切」，只返回 `function onCheckedKeys(`；**后果比「少验一点」严重**：往里写任何 `rewrites` 都会抛「声明的改写失效」，而**不写规则、任它绿**才是最危险的 —— 那个「✓ 逐字一致」是假的。
| 9a | **怎么修的**（Task 3.5）：比对核心抽成 `docs/home-audit/lib/extract-movecheck-core.mjs`（两份守卫从此只有一份实现，等于解掉了「改一份 vs 别碰另一份」的死结），`sliceFn` 的终点判据重写为**四条路**：① 单行声明（括号收支 ≤0 **且行尾不是续行记号**）；② 括号栈跨行找函数体（只在**栈空**时遇换行才考虑停）；③ 找不到函数体 ⇒ 切到**语句末**（栈空 **且本行末尾、下一行行首都不是续行记号**）；④ 兜底退回首行。另加两条：**字符串与注释在每一处扫描里统一跳过**（`skipInert`）、**结构性硬闸**（切出来不配平就抛错、绝不拿去比对）。全表见该文件头。
| 9b | **实测的波及范围**：整个 `Home.vue` 里**顶层 `function` 声明、参数表跨行**的只有 **3 个** —— `onCheckedKeys`(REF:1601，B10) · `renderEditable`(REF:2807) · `headerFilter`(REF:2866，后两个属 B12)。**不是 3 个之外的 4 个**：`open`(REF:1500) 是 `onOpenDoc`(REF:1496) **函数体内的局部箭头函数**，既不是 `function` 声明也不是顶层，**不该单独登记**（嵌套件靠外层声明的整段比对覆盖）。修好后三者实测切 **14 / 24 / 98 行**，与**真 TS 解析器**（`app/node_modules/typescript`）取出的声明文本**逐字相同**；B10 的 `onCheckedKeys` 修好后**第一次被真正比对**，当场报出一处此前看不见的真差异（已按规矩**补 `rewrites` 规则**，不是删名字）。
| 9c | ⚠️⚠️ **同一次复核查出的更坏情况：Hui 侧有 4 个「已登记、报了逐字一致」的名字是假绿。** `sqCell`(11 行) · `cCol`(2) · `sub`(5) · `orderNoCell`(2)（都在 `hui-extract-movecheck.mjs` 的 `MOVED_TO_COMPONENT` 里、**不在** `POST_MOVE_EDITS`）—— 都是 `const NAME = (…) =>` 这种**表达式体箭头**，首行括号正好配平 ⇒ 命中「单行声明」快车道 ⇒ **只比了第一行**，函数体一行都没比。根因就是上面①里「行尾续行记号」那条判据当时不存在。修好后**全文重比：4 个都是真的逐字一致** —— 恒等式的结论没变，但这 4 条绿从「空绿」变成了「真绿」。**这就是「守卫报绿 ≠ 这段验过」的实证。**
| 9d | **复核用的独立判据（可复现）**：真 TS 解析器 —— `ts.createSourceFile` 后**递归**遍历整棵树取同名声明的 `getText()`，与 `sliceFn` 结果都过一遍 `norm()` 再比（必须递归：搬进工厂函数后这些声明不再顶层，只看顶层会把目标文件全漏掉）。2026-09-20 用它复核了双方清单里**每一个**名字 —— Home **13** 个（参照 + 目标 = 26 个切片点）+ Hui **93** 个（47 第一轮 + 46 第二轮，= 186 个切片点）。命令与输出见 `task-3.5-report.md`。**`npm run build` / `vue-tsc` 都抓不到这类问题**（它们只看类型，不看「这段代码有没有被比对过」）。<br>⚠️ **这个判据自身踩过三个坑，用之前先看**（2026-09-20 全部修掉，修之前报出的数字不可信）：① 遍历时**必须限定在「可整体搬走的声明」**这几种 kind —— 否则 `engine` 会命中 `{ engine?: OrderLines }` 里的 `PropertySignature`，报出**假的不同**；② `.vue` 文件**不能整个喂** `createSourceFile`：最后一个语句的 `end` 会越过 `</script>`（`<style scoped` 被当成类型断言续上去），解析器文本凭空多 6 行 ⇒ 要先只取 `<script …>…</script>` 那段；③ `VariableDeclaration.getText()` 起于 `const`、**不含 `export`**（`export` 挂在外层 `VariableStatement` 上），必须**上提一层**才能与 `sliceFn`（它从 `export` 那格开始切）对齐。修好这三点后重跑：清单内 **106 个名字全部合格（切全 + 配平 + 与解析器逐字相同）**，零异常。 |
| 9e | **残留（登记前先看）**：`sliceFn` 是文本切片器、不是解析器。残留有**两族**（2026-09-20 Task 3.6 补记：本节原先只写了正则那一族，读的人会以为残留只有一种）——<br>🔴 **🅰️ 泛型实参里的类型字面量 —— 已修（Task 3.6，来源 `task-3.5-review.md` 的 🔴1）**：`reactive<{ … }` / `ref<{ … } \| null>` / `defineProps<{ … }>()` 这类**多行**类型字面量，其 `{` 在扫描时「栈空 + 后面紧跟换行」两条全中 ⇒ 被当成**函数体** ⇒ 从它配平到类型字面量的 `}` 就返回 ⇒ **初值整段被丢掉**。最坏的是切出来那段**括号是配平的** ⇒ **结构性硬闸也拦不住**（与下面 ① 同属「配平但切短」）。**实测的假绿**：把 REF 里 `queryForm` 的初值 `onlyProduction: false` 改成 `true`，`firstDiffLine` 仍是 **0**（守卫报「逐字一致」）。受害真形状**四处**（2026-09-20 Task 3.7 改准：**四处只是「需要立刻保护的那四个」** —— `queryForm` 会被 Task 8 登记、`props`/`emit` 在 Hui 守卫的目标文件里、`financeOrder` 不属任何块；**不是同族的全部**）：Home `queryForm`(REF:1237–1243) · `financeOrder`(1388–1394) · `DetailLinesTable.vue` 的 `props`(68–112) · `emit`(114–119)（修前四者的切片末行都是光秃秃一个 `}`，丢掉了 `>({ … })` / `\| null>(null)` / `>()`）。<br>⚠️⚠️ **同族实际修好 70 处，形状至少四种** —— 上一版这里（与守卫文件头）把同族的 `const x = {` + 换行、`type X = {` 写成**「等价、不在残留里」**，**那句话是错的**：实测（旧核 `bc8d5855` vs 新核 `970369c3`，全 `app/src` 154 文件 / 3645 个非重名声明）**旧错→新对 70 条** = `defineProps<{`/`defineEmits<{` **42** + `const X = {`/`type X = {` **24** + `reactive<{`/`ref<{` **3** + `const HEADER_ROWS: {…}[] = [` **1**（`Receipt2SettingsDialog.vue:277`，整段数组初值被丢，4 行 → 8 行）。**触发条件**：多行字面量收尾的 `}` **同一行后面还有文本**（` as const` / `as const satisfies …` / ` & B`）时旧核切到 `}` 就返回、新核切全；只有收尾 `}` 就是声明末尾时两侧才逐字相同。反例 4/4：`printService.ts::HiprintModule`（丢 ` & Partial<HiprintCtor>`）· `docsheet/defaults.ts::PAPER_UI_RANGES` · `receipt2/defaults.ts::FONT_RANGES` · `productionsheet/profile.ts::PS_CLASSES`（后三个丢 ` as const …`）。⚠️ **这 4 条的反例行数一模一样、只有内容能区分** —— 别用行数证明这件事（本项目栽过一次）。复跑：`node docs/home-audit/decl-sweep.mjs --against bc8d5855`。<br>🔴 **复跑命令与本节数字对不上，必须这么读**（2026-09-20 Task 3.8 改准，Task 3.7 复审 §2🟠3 抓出）：`--against` **只认「旧核」那一个 ref**，**新核恒为工作区**，它**没有**「指定新核」的参数 ⇒ 「旧核 `bc8d5855` vs 新核 `970369c3`」这种**声明的 sha 对，用这条命令根本表达不出来**。同一对 sha 在不同 HEAD 上跑出的数**不一样**，实测：<br>· 新核 = `970369c3`（把工作区的 core 换成那个提交的）⇒ **旧错→新对 70**（**这才是本节那个 70 的口径**）<br>· 新核 = `9181ea0b`（Task 3.7 的笔）⇒ **100**（= 70 + Task 3.7 自己修好的 30）<br>· 新核 = **当前工作区**（Task 3.8 之后，含 `'>'`）⇒ **102**（= 100 + `'>'` 那 2 处）<br>⇒ **照抄这条命令看到的是 102，不是 70** —— 那不是本节写错了，是**新核变了**。想复现某个历史状态，得先把工作区的 core 换成那个 ref 的再跑。<br>⚠️ 另：这条命令的**分母只有 3640 个声明**，**794 个它完全看不见** —— 见 #9h 的「盲区」表。⚠️ **但「形状四处」≠「四张假绿」—— 今天会被真绿卡盖住的恰好一个**：`queryForm`（`task-8-brief.md` 的 `consts` 里就列着它）。另外三处都不构成假绿：`financeOrder` 不属任何块；`props`/`emit` 所在的 `DetailLinesTable.vue` **是** Hui 守卫的目标文件（`hui-extract-movecheck.mjs` 的 `COMPONENT`），但这两个名字**今天没被登记**（在该文件里 grep `'props'` 与 `'emit'` 均无命中）⇒ **谁把它们登记进去，那一刻就变成假绿**。所以本洞的性质不是「绿卡已经坏了」，而是「**发绿卡的那台机器会把这一类全盖成绿的**」。<br>**规模（2026-09-20 实测，`app/src` 全仓库）**：`defineProps<{` / `defineEmits<{` / `reactive<{` / `ref<{` 共 **74 处**（其中 `defineProps` **35** + `defineEmits` **32** —— 这是本仓库写带类型 props/emits 的**主流写法**）；其中 `{` 处**括号栈为空**的 67 处，再叠上「`{` 后紧跟换行」= **52 处**，才是修前**真的会被切错**的那一子集。⚠️ **74 是「形状出现次数」，不等于「74 处都会被切错」**，两个数别混。**实测不算本洞的三类**：`allocs: Array<{`（Home REF 1821 与 `useHomeSelection.ts:353`，两侧同形）上面压着 `interface CGroup {` 的 `{` ⇒ 栈非空（`clearAccounts` 修前修后都是 **79 行、逐字相同**）；Home REF 2684 的 `rowTipLines` 是**单行**类型；Hui 侧 `Hui.vue` 1401 / 3323 与 `useOrderLines.ts:195` **全是单行类型 ⇒ Hui 侧无受害者**。<br>**修法**：函数体 `{` 加一条**顺序**要求 —— 必须出现在一个**把括号栈弹空的 `)`** 之后（核心 `sawParen` 闩，见 `lib/extract-movecheck-core.mjs` 文件头「路 2」第 4 条）；挡掉的那些 `{` 落回「push 进栈」，由**路 3（语句末）**整条切出。修后上例 `firstDiffLine` = **7**（真红）。⚠️ **`task-8-brief.md` 的登记清单里有 `queryForm`** —— 不修的话那一笔落下去就是一张**空绿卡**。<br>🅱️ **不认正则字面量 —— 仍未修**，它有两种表现（2026-09-20 各自都实测过；但**仓库里只有 ② 有真实实例**，① 目前只有自测里的合成形状）：<br>① **里的括号**：`const re = /}/` 之类不进括号栈 ⇒ 函数体**提前收口**，而收口后的片段**是配平的**（**结构性硬闸也拦不住**这条）—— 这是最危险的一种。⚠️ **但「配平但切短」并不是正则独有的**：上面 🅰️ 那个泛型实参的 `{` **用普通代码**就造出同一种后果，而且它**已经活在仓库里**（反倒正则这条一处都没被登记）—— 别把本条读成「只有正则才危险」。<br>② **里的 `//`**：`replace(/\//g, '-')` 的字符序列是 `/ \ / /`，`skipInert` 把第三个字符起当成**行注释**、跳掉整行余下部分（**含收尾的 `)`**）⇒ 收支多 1 ⇒ 硬闸抛「切片不平衡」。**真实实例 `app/src/views/Progress.vue:2149` 的 `exportStamp`**（全仓库仅此一处）。⚠️ 这一例的切片其实**是对的**（14 行、与解析器逐字相同）—— **错的是判据不是切片**，且表现是「响亮拒绝」而非「静默放过」，所以比 ① 安全，但它会挡住该声明被登记。<br>两种都**没修**（认正则要先判 `/` 是除号还是正则起点，属另一档改动，**待裁决**）。**登记状态**：正则这两族**今天一处都没被登记**（全仓库唯一的**真实**实例是 ② 的 `Progress.vue:2149` `exportStamp`，不在任何名单里；① 目前只有自测里的合成形状）；泛型那族**正相反** —— `queryForm` 就躺在 `task-8-brief.md` 的 `consts` 里，**差一点就以一张空绿卡落地**，这正是本条被判**阻断级**的原因。<br>⚠️ **顺带更正一处「同一件事两种说法」**：① 那半**只有收支变负的写法会被硬闸拦下** —— `/)/` 让收支变负 ⇒ 硬闸响亮报错；`/}/` 是**提前收口且配平** ⇒ **硬闸不报**。别把这条概括成「硬闸会挡住正则」（核心文件头此前那句就是这个毛病，Task 3.6 已一并改准）。<br>其余边界（`${}` 与内层反引号、嵌套件、快车道）见核心文件头「能力边界」。 |
| 9f | **「首行括号配平、声明却在下一行继续」这条残留——一半已修（`d54e9531`/`44b42bc2`），另一半 Task 3.7 才修**。<br>⚠️⚠️ **本节此前写的是「已修」，那是错的一半**：当时造的**两张记号表**（行尾续行记号 / 下一行行首续行记号）**只被用在路 3 的 lookahead 里**，而**路 1（单行声明快车道）一次都没调用过 `headContinues`**，行尾表当时又缺 `<` ⇒ 这一类仍然被**静默切短**：首行括号收支 ≤ 0、行尾不在表里 ⇒ 直接返回首行，而 `fast: true` 让 `sliceFn` **跳过结构性硬闸** ⇒ 切出 **1 行、配平、不报错、拿去比对**（守卫会报「逐字一致」）。**修前实测 33 处**（含 **6 处顶层**：`docSheetUi.ts` 的 `DocSheetRenderApi`(22 行)/`DocSheetUiApi`(10)/`DocSheetDialogProfile`(24)/`DocSheetUiProfile`(46)、`productionSheetUiProfile.ts::ProductionSheetUiProfile`(5)、`qualifiedLabelUiProfile.ts::QualifiedLabelUiProfile`(5)；⚠️ 复审报告量到的是 **38 处**，差额 5 处全是**同名重复**声明，Task 3.7 已逐条对上 —— 见 #9h），形状最多的是「下一行以 `.` 或 `?` 开头」（`printPayloads.ts::{main,src,casing,suppress}`、`partsEngine.ts::bad`、`docsheet/paginate.ts::{theadCells,bodyRows,tds}`…）。**Task 3.7 修法**：路 1 判据加 `&& !headContinues(…)`（看下一行行首）+ `TAIL_CONTINUES` 补 `'<'`，并各补断言（`--selftest` 29 → **32** 条）。<br>**Task 3.7 修后实测**：33 → **3 处**；方向表 `--against 970369c3` 为 **旧错→新对 30 / 旧对→新错 0**。<br>🔴 **Task 3.8 收口后（当前状态）：「切短」= 0** —— 但**别把这读成「全绿」**（「全绿」这个词上一轮就是错的）。这个 0 是**三件事合起来**的结果，逐条说清见 **#9h 的「Task 3.8 收口」段**：<br>① **1 个真修** —— `HEAD_CONTINUES` 补 `'>'`（泛型实参表收尾的 `>` 独占一格），逐字相同 **3505 → 3507**、旧错→新对 **30 → 32**；<br>② **1 个归类** —— `DetailLinesTable.vue::old` 那条是 **ASI 守卫 `;` 的「口径差」**，**不是洞**（切片含着完整声明），`decl-sweep` 从「切短」挪进新的一栏「口径差」，**不判红**；<br>③ **1 个明确不加** —— **`';'` 被实测否决**：加了它「逐字相同」**一条都不涨**（3505 → 3505），只是把那条「短」挪进「多带」桶 ⇒ **闸绿了而切片仍然错**。那是**洗白**，不是修法（依据见 #9h）。<br>⚠️ **上表那「四族」里还有两族是「潜在、仓库里 0 处」**（正则 `}` · 路 2 的 `}.成员()` 收尾）⇒ **「切短 = 0」≠「切片器已经没洞了」**。<br>下面这段是**原来那一半**（`d54e9531`/`44b42bc2` 确实做掉的部分，保留）：判据是两张记号表：**行尾续行记号**（`= => \| & && \|\| ?? + , . ? : ( [ {`，看**最后一个代码字符**，`lastCode` 保证不被行尾注释骗）+ **下一行行首续行记号**（`\| & . ? :`，专治 prettier 把 `\|` 写在下一行行首的联合类型）。实测：`type TextFilterKey`(REF:769) 旧核切 **1 行**、现在切 **10 行**，**只改它的第二行**旧核判「一致」（diff=0）、现在**报红在第 2 行**；`sliceFn` 也认 `type` / `interface`。另修了「声明收口在**文件尾**（末尾无换行）」与「后面还有换行」**两侧不等价**（`fixtures/hui-pre-3b.ts` 的末声明 `diaoColumns` 就是无换行收口）—— 不修则两侧一个 3 行一个 1 行，比出一个**切片器自造的假差异**。三条都已在 `--selftest` 里各有一条**改坏即红**的断言钉住（本节写作时共 **22** 条；**Task 3.6 后 29 条**；**Task 3.7 后 32 条**；**Task 3.8 后现共 36 条** —— 见 #9g / #9h）。 |
| 9g | ✅ **【已修，2026-09-20 Task 3.6】洞④：泛型实参里的 `{` 被当成函数体**（详情见 #9e 的 🅰️）。同笔还关掉三条（都在同一批复审里点出）：<br>① **🟠3 快车道 `depth <= 0` 没被钉住**：把 `<= 0` 改成 `=== 0` 时 `const re = /)/` 从「1 行」变成**抛错**，而 `--selftest` 此前**仍然 EXIT=0**（原先把收支**0** 的那条当成覆盖了这条）。⇒ 补断言「负收支单行声明切出 1 行且不抛」。<br>② **🟠4 `headContinues` 的「跳过注释行」没被钉住**：去掉跳注释后，联合类型中间夹一行 `//` 会在注释处停住，而自测仍全绿。⇒ 补断言。⚠️ ~~这是**潜在**不是**现存**（仓库里 **0 处**这种形状）~~ —— **这句写错了，Task 3.7 改准**：它是**现存 3 个声明**（2026-09-20 **Task 3.8 实测改准 —— Task 3.7 写的「1 个」也是错的**，这一段至今**错过三次**：`task-3.6-report.md` §6🟠4 的「0 处」→ Task 3.7 的「1 个」→ 本次实测「3 个」，**以本次为准**；这又是「**复核本身也会错**」的一例 —— Task 3.7 的订正把 0 改成了 1，**仍然是错的**）：<br>· `app/src/utils/docsheet/profile.ts:79-85` **`CellKind`**（7 行；成员之间夹 `/** … */`）<br>· `app/src/utils/printPayloads.ts::src`（10 行）· `app/src/utils/printPayloads.ts::suppress`（6 行）<br>⇒ 断言 ⑰（+ Task 3.8 补的 ㉒）**在守三个现存声明**，而且**两种注释都要跳**。**三个现在都是对的**（去掉「跳注释行」这一半后，三个都从「逐字相同」变成「切短」，行数 7/10/6 → 3/3/3）。<br>⚠️ **量法**（别再用「扫一遍看看有几处」）：在 `/tmp` 副本里把 `core` 的 `if (t === '' \|\| t.startsWith('//') \|\| t.startsWith('/*') \|\| t.startsWith('*')) {` **整行**改成 `if (t === '') {`（三个注释分支全去），跑**全部**声明，看切片文本变了的那些 —— **恰好 3 个**（分母 = **3640** 个非重名可比声明）。<br>⚠️ `src`/`suppress` 这两条**又一次现形了 §1b 陷阱**：不加 `norm()` 时「现核 10 行 vs 真声明 10 行、内容不同」，**行数一模一样**；**只有过 `norm()` 才看得出相同** —— **别用行数论证这件事**。<br>（`task-3.6-report.md` 也写着「0 处」，那是**已提交的历史记录、故意不改**，以此处为准。）<br>③ **🟡5 Hui 守卫补一层 `try/catch`**（两轮循环各一层）：此前 `computeSquare` 体里塞一个 `/)/` 会让脚本**未捕获异常、第一条循环就中止**（47 个比对与组件侧 46 个全不跑）。**这不改变任何绿**（`fail.push + continue` ⇒ 退出码仍 1）—— 实测「加之前是崩、加之后是失败行」，两种都非 0。Hui 侧的判据/清单/`REWRITES`/文案**一字未动**。<br>**回归证明（本次的命门）**：Home 13 名 + Hui 93 名（47 第一轮 + 46 第二轮）在修前/修后的切片**逐字零差异**（212 条切片点 + 0 异常）；把范围放宽到 4 个源文件里**全部 966 个声明名**，切片**只有 4 条变化，全是上面那四个受害者**；`const x = {` + 换行、`type X = {`、`computed<…>(() => [` 这几族实测**与修前逐字等价**。⚠️⚠️ **这句里前两族是错的，Task 3.7 改准**：`const X = {` + 换行、`type X = {` 这两族**不等价**（24 处旧核切短、新核切全，触发条件是收尾 `}` 同行后面还有 ` as const` / ` & B` 之类文本）；只有 `computed<…>(() => [` 那一族确实等价。**「整个仓库」这个词也超出探针范围**（探针只覆盖 7 个文件 / 966 个声明）：全 `app/src` 154 文件 / 3645 个非重名声明里，本笔使 **70 条**由「旧错」变「旧对」（另有 1 条口径噪声）—— 逐条见 #9e 与 `task-3.7-report.md`。⚠️ `task-3.6-report.md` §4 的「整个仓库里只有那 4 条变」同此**超出探针范围**，那是**已提交的历史记录、故意不改**，以此处与 #9e 为准。命令与原始输出见 `task-3.6-report.md`。 |
| 9h | ✅ **【已修，2026-09-20 Task 3.7】洞⑤：路 1（快车道）不看下一行行首 ⇒ 33 处静默切短（含 6 处顶层）**。<br>**修前形状**：`sliceFn` 的路 1 只判三条 —— 首行括号**收支 ≤ 0**、**首行没有未闭合的串**、**行尾最后一个代码字符不在续行表里**。三条全中就直接返回首行。于是「首行括号正好配平、声明却在**下一行**继续」的声明被切成 **1 行**；更坏的是 `fast: true` 让 `sliceFn` **跳过结构性硬闸**（`if (r.text != null && !r.fast)`）⇒ 切出来**配平、不报错**，守卫照样报「逐字一致」⇒ **假绿卡**。两张记号表当时**都已经存在**，但只在**路 3 的 lookahead** 里被调用过 —— 「表在那儿」≠「这条路上用了它」。<br>**修前实测**（`decl-sweep.mjs`）：**33 处**静默切短，含 **6 处顶层** —— `docSheetUi.ts` 的 `DocSheetRenderApi`(22 行) / `DocSheetUiApi`(10) / `DocSheetDialogProfile`(24) / `DocSheetUiProfile`(46)、`productionSheetUiProfile.ts::ProductionSheetUiProfile`(5)、`qualifiedLabelUiProfile.ts::QualifiedLabelUiProfile`(5)；嵌套 27 处，最多的是「下一行以 `.` / `?` 开头」。⚠️ 复审报告写的是 **38 处**：差额 5 处全是**同名重复**声明（`DetailLinesTable.vue::{next,selected}` · `productionStats.ts::dates` · `printPayloads.ts::lockway` · `ProductionSheetLayoutDialog.vue::totalWidth`，各自在同文件里出现 **2 次**）—— `sliceFn` 只切**第一个**同名声明，`decl-sweep.mjs` 把这种**跳过**（33 + 5 = 38 ✓，两侧差集为空）。<br>⚠️ **口径**：这个等号只在「**每个文件里同名声明只算第一个**」的口径下成立 —— **仪器现行口径是「整组同名一起跳」**，所以**修前**它报的是 **33 处**（就是本节头条那个数）。<br>⚠️ **其中 `DetailLinesTable.vue::next` 变短的机理**不是**重名**：「同文件出现 2 次」只解释它**为什么会出现在这份差额里**；它**变短**是因为它的**第一处**（`:335`）**本身就是短切片**，机理与 `old` **完全相同** —— `335: const next = (v as string) ?? ''` / `336: ;(l as unknown as Record<string, string>)[field] = next`，解析器把 `336` 行行首那个 **ASI 守卫 `;`** 算进了上一条声明。<br>**修法三处**：① `core` 路 1 判据加 `&& !headContinues(src, firstNl < 0 ? src.length - 1 : firstNl)`（看**下一行行首**）；② `TAIL_CONTINUES` 加 **`'<'`**（泛型断行 `export interface X<` 的首行收支正好 0，`<` `>` 不在括号栈里 —— 合法声明不可能以 `<` 收尾，`const a < b` 也不是完整语句）；③ `--selftest` 补 **3 条**「改第 2 行必须报红」的断言（成员链 `.` / 三元 `?` / 泛型 `<`）。⚠️ ③ 不是可选项：**M-a/M-b 实测**证明只加 ① ② 而不补断言时，现有断言**一条都不会变红**（改坏也没人知道）。**变异证明**：M-a（去掉 `headContinues`）⇒ ⑱⑳ 红 · M-b（去掉 `'<'`）⇒ ⑲ 红 · M-ab ⇒ 三条全红；两条修法各有一条**独立**的钉，无弱断言。<br>**Task 3.7 修后实测（历史）**：33 → **3 处**（`decl-sweep.mjs`；方向表 `--against 970369c3` = **旧错→新对 30 / 旧对→新错 0**）。⚠️ **这 3 处当时是已知残差、不是「已清零」**：① `DetailLinesTable.vue::old`；②③ `productionSheetUiProfile.ts::ProductionSheetUiProfile` 与 `qualifiedLabelUiProfile.ts::QualifiedLabelUiProfile`（修后从 1 行 → **4 行**，仍丢收尾那行独占一格的 `>` —— 路 3 的「语句末」不把裸露的 `>` 当续行）。三处**都各少 1 行**，且都**配平、不报错**。**Task 3.7 按它那份简报的范围只做了三处中的修法、未动这两条**（会扩大回归面，与「路 2 不许动」同性质），**交控制器裁决** —— **裁决结果 = Task 3.8 做掉了 ②③，并证明 ① 不是洞**（见下「Task 3.8 收口」）。<br>🔴🔴 **订正一处写错的机理**（2026-09-20 Task 3.8，Task 3.7 复审 §2🟠2 抓出）：本节此前把 ① 写成「`DetailLinesTable.vue::old`（**下一行只有孤零零一个 `;`**）」—— **「孤零零一个 `;`」是 `ts.getText()` 的呈现，不是源文本**。源文件那一行是完整的 `;(l as unknown as Record<string, string>)[field] = (v as string) ?? ''`（`awk 'NR==204' app/src/components/DetailLinesTable.vue` 可验）。<br>🔴🔴 **同时订正「①与②③是同一个家族」这句 —— 它是错的**：②③ 是**真洞**（缺 `'>'`，切片确实少切了一行），**① 不是洞**（切片是对的，是**解析器口径**把下一行行首的 ASI 守卫 `;` 算进了上一条声明）。<br>⚠️ **把 ① 说成「同类洞、只是先不修」，会让人以为它在等一个修法 —— 它永远等不到，而且唯一「能修」的那条路（加 `';'`）是洗白。**<br>**本任务留下的仪器**：`docs/home-audit/decl-sweep.mjs` —— 真 TS 解析器三方对照（全 `app/src` 154 文件 / **4434** 个可识别声明；其中同名重复 **789** 个被跳过 —— 4434 − 789 = **3645**，与 #9e 那张表同口径），`--check` 把「解析器切得全、`sliceFn` 切短」变成**硬闸**（exit 1），`--against <sha>` 打「旧核 vs 新核」方向表（**旧对新错必须是 0**）。⚠️ 它验的是**工具本身**（切片器切没切对），**不是「搬迁有没有搬错」**；也**故意没接进 `run-all.mjs`**（接不接由 Task 15 / Task 29 裁决）。<br>⚠️ **路 2 对 `}.成员()` / `} satisfies X` 收尾仍未修**（**潜在、仓库里 `^\s*}\.` 0 行**）：函数体 `}` 同一行后面还有文本时会在 `}` 处收口，而且**配平** ⇒ 硬闸不报。**只记账，本任务没动路 2 的收口逻辑。**<br><br>---<br>### Task 3.8 收口（2026-09-20）—— 把 3 处残差里的**真洞**修掉、把**假修法**钉死<br>**① 修法（唯一的真修）：`HEAD_CONTINUES` 补 `'>'`**<br>`interface X<` / `type X = Y<` 的**泛型实参表**在 prettier 下会把收尾的 `>` **单独留在下一行**，而中间几行收尾既不是 `<` 也不在行尾表里 ⇒ 只有**看下一行行首**才拦得住。`>` 同样**不可能是顶层声明的开头** ⇒ 加它是**纯收窄**（与 `TAIL_CONTINUES` 补 `'<'` 同性质）。<br>**实测**：逐字相同 **3505 → 3507**、切短 **3 → 1**、方向表 `--against 970369c3` 旧错→新对 **30 → 32**、**旧对→新错 仍为 0**。修好的两条：`productionSheetUiProfile.ts::ProductionSheetUiProfile`（收尾 `>` 在**第 44 行**）· `qualifiedLabelUiProfile.ts::QualifiedLabelUiProfile`（**第 70 行**）。断言 **㉑** 钉住它（去掉 `'>'` 即红）。<br><br>**② 🔴🔴 `';'` 是「洗白」不是修法 —— 后人别再去试**<br>剩下那 1 处（`DetailLinesTable.vue::old`）看起来「差一个 `;`」，加 `';'` 进 `HEAD_CONTINUES` 也确实能让「切短」归零 —— **但那是假的**。三组**分开**量的实测（把 `HEAD_CONTINUES` 分别改成 A/B/C，跑完整 sweep）。下表是**现行四类仪器**的读数（「现状」= BASE `9181ea0b` 的核，即还没加 `'>'` 时）：<br><br>· **现状** —— 改动：无 · 逐字相同 3505 · 多带 132 · 切短 2 · 口径差 1 · `--check` exit 1<br>· **A** —— 改动：只加 `';'` · 逐字相同 **3505（没涨！）** · 多带 **133** · 切短 2 · 口径差 **0** · `--check` exit 1<br>· **B** —— 改动：只加 `'>'` · 逐字相同 **3507（+2）** · 多带 132 · 切短 **0** · 口径差 1 · `--check` **exit 0**<br>· **C** —— 改动：两个都加 · 逐字相同 3507 · 多带 133 · 切短 0 · 口径差 0 · `--check` exit 0<br><br>⚠️ **同一批数据在「三类」时代的读法是 切短 3 / 2 / 1 / 0**（控制器最初就是这么量的）：那时**还没有「口径差」这一栏**，`DetailLinesTable.vue::old` 被算在「短」里，所以 B 组当时也是 `exit 1`。**加了第四类之后 B 就 `exit 0` 了** —— 因为剩下的那 1 处是**口径差不是洞**。**两个读法都对，只是口径不同**；引用时**必须写清用的是哪一套**。<br>看 A 组那一行（**两套读法下结论一致**）：加 `';'` 之后 **「逐字相同」一条都没涨**，`old` 那一处只是从「口径差」**挪进了「多带」桶**（132 → 133 / 口径差 1 → 0）—— **换了个桶，切片还是错的**。<br>🔴 **第二个独立佐证：方向表上 A 组的「改进」也一条没涨**（旧核 `970369c3`，四组各跑一次）：现状 **30** · **A（只加 `';'`）30** · B（只加 `'>'`）**32** · C（两个都加）32，`旧对→新错` 四组**全 0**。⇒ `';'` **既没让任何切片变对，也没让任何切片变错** —— 它只改变了**分类的桶**，这正是「洗白」的定义。<br>**机理**：`app/src/components/DetailLinesTable.vue` 是 **ASI 守卫**写法 —— `203: const old = … ?? ''` / `204: ;(l as unknown as Record<string, string>)[field] = (v as string) ?? ''`。真 TS 解析器把**第 204 行行首那个 `;`** 算作上一条 `VariableStatement` 的**终结符**（`stmt.getText()` = 第 203 行 + `\n        ;`），而 `sliceFn` 只切到第 203 行 ⇒ 被判「短」。加了 `';'` 之后，路 3 的判据（`core:476` 的 `!headContinues(src, k)`）**不再在第 203 行末收工**，于是**整条第 204 行被吞进切片**，切片比解析器文本**还长** ⇒ 掉进「多带」。<br>⚠️ **这正是这套守卫最怕的那件事：切片仍然是错的，但闸变绿了。** 它与「不许为了让测试变绿而改夹具」是同一条红线。**⇒ `';'` 明确不加。**<br><br>**③ 新增第四类「口径差（ASI 守卫 `;`）」**<br>判据（在 **`norm()` 之后**的文本上算）：`norm(解析器文本).startsWith(norm(切片))` 且**余下部分**匹配 `/^\s*;$/`。<br>**为什么不算洞**：**切片含着完整的声明**，去掉的只是一个 **ASI 守卫符**（不是声明的一部分）⇒ **任何真改动都会被切片抓到**，判别力不受影响。这与「静默切短」有本质区别。<br>**它只有一个实例**：`app/src/components/DetailLinesTable.vue::old`。<br>**`--check` 的新语义**：**「切短」为 0 即通过**；「口径差」照常打印、照常计数，**但不判红**。<br>⚠️ 放松闸门的那一行**必须有断言证明它没把真洞一起放过去** ⇒ `decl-sweep --selftest` 合成⑤（**近邻负例**：差值以 `;` 开头但后面还有东西 ⇒ **仍然进「切短」**）+ 合成①（真切短仍红）。<br><br>**④ 现状：「切短」= 0**（Task 3.7 的 3 已归零）。**但这不是「全绿」** —— 那个 0 = **1 个真修（`'>'`）+ 1 个归类（`old` → 口径差）+ `';'` 明确不加**。上表「四族」里另有两族是**潜在、仓库里 0 处**（正则 `}` · 路 2 的 `}.成员()` 收尾）⇒ **「切短 = 0」≠「切片器已经没洞了」**。<br><br>**⑤ `--against` 的分母与盲区（🔴 报「旧对→新错 = 0」时必须带上分母）**<br><br>· 可识别声明总数 —— **4434**<br>· `--against` 实际比较的 —— **3640**（= 4434 − 789 同名重复 − 4 切不出 − 1 抛错）<br>· **它完全看不见的** —— **794（17.9%）**<br><br>这 794 的构成（`decl-sweep.mjs` 里 `countByName` 那几行）：`countByName` 是**按文件**统计的，判据 `if (countByName.get(nm) > 1) { … continue }` 把**同文件里重名的声明全部跳过、包括第一个** —— 其中 **495 个**是第二次及以后的出现（`sliceFn` 本来就只切第一个，比不了，**无可救药**），另 **294 个**是**同名组的第一个**（**本来能被切、只是被这条判据多跳了**）。495 + 294 = 789，再加 4 + 1 = **794**。<br>⇒ **「旧对 → 新错 = 0」不是全仓结论**，它只覆盖 3640 个。那条跳过规则**本任务没改**（改了会重新基线化 33 / 3 / 3640 这一整套头条数字）⇒ 只记账，归 Task 29。<br>**另：`--against` 现在会在「旧对→新错 > 0」时 exit 1**（此前 `§9h` 把「必须是 0」写成硬要求，但**仪器不会因此变红** ⇒ 那是纸面的「必须」）。当前两对 sha 这一项都是 0 ⇒ **改完仍是绿的，不是红着上线**。<br>⚠️ `--against` **没有「指定新核」的参数**（新核恒为工作区）—— 见 #9e 的复跑说明。<br><br>**⑥ 断言覆盖的订正**：⑰ 此前**只钉了 `//` 那一半**，而文档白纸黑字写着「`//` 与 `/** *\/` 两种注释都要跳」——**声称强度大于断言强度**。实测把判据改成只跳 `//` 后 `--selftest` 仍 **EXIT=0 · ✓32 ✗0**，而 `CellKind` 的切片**从 7 行变成 3 行**。⇒ Task 3.8 补 **㉒** 专钉 `/** *\/`；另补 **㉓**（`&` 交叉类型断行）与 **㉔**（跳过空行）—— 这两条判据此前同样是**改坏也没人知道**（各实测 EXIT=0 · ✓32 ✗0）。`--selftest` **32 → 36 条**。<br>⚠️ 诚实说明：**㉑ 不是「只在自己的变异下红」** —— 它的样本首行以 `<` 收尾，所以 `TAIL_CONTINUES` 去掉 `'<'` 时它**也会红**（实测：去掉 `'<'` ⇒ ㉑ 与 ⑲ 两条红）。这是形状本身决定的，不是弱断言。<br><br>**⑦ 记账：`9181ea0b` 的 commit subject 里那个「38 处」**（**不改那个提交** —— 它是在飞复审的 HEAD，amend 会让复审手里的包失效）：那个「38」**是另一口径下的正确读数**（= 「每个文件里同名声明只算第一个」），**本仪器头条数一律用 33**（= 整组同名一起跳）—— **两个数都对，只是口径不同**，口径说明见本节上面那句。根因是那份简报把「38 处」写进了它 mandate 的 `git commit -m` 模板、**却没写口径** —— **简报里不许再出现没实测过的数字，尤其不许出现在 `git commit -m` 模板里**（Task 3.8 的 commit subject 刻意**不含任何数字**）。 |

| 10 | **B1 实测 vs 方案**（2026-09-20，实做后补记）。① **段界比方案小 17 行**（量法：方案口径 `538–576` + `1540–1548` = 39 + 9 = **48** 行，实测口径 `538–544` + `546–560` + `1540–1548` = 7 + 15 + 9 = **31** 行 ⇒ 48 − 31 = **17**；也可拆成被跳过的 `545` / `561` 两行空行 + `562–576` 那 15 行）：方案写 `538–576`，实测搬的是 `538–544 + 546–560 + 1540–1548`。差的 `562–576` = **`onMounted`(562–571) + `onLogout`(573–576)**，**裁定留页面** —— `onMounted` 体内 `563 auth.loadMe()` · `564 router.push` · `567 load()` · **`570 homeFormulas.value = f`**，而 `homeFormulas` 属 **B6**：跟着搬会让 B1 **反向依赖 B6**（将来 B6 又得反过来注入 `load`，成环）。留页面零额外代价 —— 它只需要本块回传的 `load`。<br>② **「4 项」是输出面、不是注入面**：实测**注入 2**（`message` · `auth`）/ **输出 6**。§3.1 那一列历来两种数混着写（B10 同病）⇒ 读它时先看该格的定性。<br>③ **一处编译期约束（第二次踩，第一次是 B10 的 `DataTableRowData`）**：`OrderFinance` 类型随 `financeSummary` 搬走后，`Home.vue` 那份 import 里它**只剩零处用处** ⇒ `vue-tsc --noEmit` 报 **TS6196 `declared but never used`** ⇒ `npm run build` 红、`verify` 整体退 1（**守卫 `home-extract-movecheck` 全绿、`npm run build` 之外的门也都是绿的** —— 只有真跑 `verify` 才看得见）。处置：删掉它 + 4 行说明注释。**⇒ 通则：搬走一个「带类型标注的声明」时，同一个 import 清单里的那个类型可能只剩那一处用处** —— 搬完**先跑 `vue-tsc`**，别只看守卫绿。<br>④ 🔴 **`npm run verify` 里第一个被「纯搬迁」绊红的台子 = `docs/roles-admin-logiccheck.mjs`**，而且**「把被数源扩到新家」这一半不够**：本块登记的注入改写 `auth.user?.` → `deps.auth.user?.`（在 `home-extract-movecheck.mjs` 的 B1 块里）**同时改掉了台子正则要的字面** —— `canSeeAllOrders(auth.user?.role)` 在新家变成 `canSeeAllOrders(deps.auth.user?.role)`，老正则数不到。**实测三档**：只扩源仍 **2**（红）· 扩源 + 正则放宽成 `(?:deps\.)?` 才 **3**（绿）· 只看 `Home.vue` 的老口径也是 **2**。**两件事必须同时做**，且**断言 `=== 3` 一个字不许改**（降成 2 = 把「查三处」降级成「查剩下的」，越搬越松）。`!canSeeAllOrders(`（`=== 2`）**本笔不受影响**（两处都在页面，看板那处是三元、没有 `!`），仍是 2；Task 6 / 8 搬另两处时按同法再跨源。<br>⑤ **⇒「纯搬迁笔不需要改台子」是错的** —— ④ 就是反例：**搬迁的 `deps.` 改写会改掉台子数的字面**，而台子数的字面**是源码文本**。这与 §8.2 #8④「改写规则必须带边界」是同一件事的两面：那一面管「别改坏别的标识符」，这一面管「**改了别的文件的锚点**」。 |
