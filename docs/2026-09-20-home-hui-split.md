...
| 9d | **复核用的独立判据（可复现）**：真 TS 解析器 —— `ts.createSourceFile` 后**递归**遍历整棵树取同名声明的 `getText()`，与 `sliceFn` 结果都过一遍 `norm()` 再比（必须递归：搬进工厂函数后这些声明不再顶层，只看顶层会把目标文件全漏掉）。2026-09-20 用它复核了双方清单里**每一个**名字（Home 26 + Hui 93 × 4 个来源）。命令与输出见 `task-3.5-report.md`。**`npm run build` / `vue-tsc` 都抓不到这类问题**（它们只看类型，不看「这段代码有没有被比对过」）。
| 9e | **残留（还没修，登记前先看）**：`sliceFn` 是文本切片器、不是解析器。当前最要紧的一条残留是**正则字面量里的括号**（`const re = /}/`）—— 它不进括号栈 ⇒ 函数体会**提前收口**，而且收口后的片段**是配平的**（结构性硬闸也拦不住）。本仓库暂无这种形状被登记。其余边界（`${}`、嵌套件、快车道）见核心文件头「能力边界」。
