...
>
> **验收** = `npm run verify`（33 台子全绿）+ `npm run build` + 搬迁件逐字一致。
> **不是**「我觉得没坏」。
>
...
| # | 问题 | 决定 |
|---|---|---|
| 1 | 这次拆分和 `ui-upgrade` 重设计的关系 | **纯搬迁，零行为变化。** 每步用 build + 33 台子验收，拆完再在干净的小文件上做重设计 |
...
2. **量小的先做**：每块搬完立刻跑全套。33 台子跑一轮约 20 秒（`npm run verify` 含
...
| **行为层**：现有 33 台子 + 新增断言 | 接线、语义、落库效果 | 根本没想到去测的角落 |
...
**本次的参照是 `28e36d21`**（2026-09-20，工作区干净，已验证 `git show` 能取到两个文件的原文）。
...
| 4 | `hui-extract-movecheck.mjs` 的 `REF` 目前是 `d6057283`；本次要把它改成能同时服务多轮搬迁的形态（先例记着「过渡期参照」的做法：拿「动手前一刻从工作区切出来的原文」当参照，并在找不到时**退回过期会话 + 打印提示，不静默放过**） |
