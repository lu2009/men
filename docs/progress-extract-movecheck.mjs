...
const BLOCKS = [
...

  {
     ...
     *   （横幅 + 10 行块注释 + 1 空行 + `MOVE_FAN_NAMES` 的 JSDoc），所以「区间含注释」不影响
     *   声明清单。1984 起已是 **P11 的段首横幅** + 它下面 13 行块注释 ⇒ **留在原地**
     *   （原写 1997 会把 P11 的横幅与那 13 行说明一起搬进 `useProgressStats.ts`）。
...

  {
     ...
     *   （横幅 + 23 行的「旧版 §4.1 链路」块注释），所以「区间含注释」不影响声明清单。
     ...
     *   ⚠️ 39 处命中**全部落在活代码里**（与文件头那张注入面表的「**20 项进 `deps`**」那部分逐名核过）——
...
if (process.argv.includes('--selftest')) {
   ...
   *    它此后不会再因为「又加了一块」而红。
   *    ⚠️ 空清单那条路（收尾打「0 条」）仍由 `total === 0` 那支给出人话解释，但**那一支现在
   *      必须退 1**（见收尾的下限自检）。`total` 是从真清单算出来的 —— 别为了「让它能被自测」
   *      去改那个分支。
   */
 ...
 *    ⚠️ 这一支**现在必须退 1**（见下面的下限自检）。**改前的理由已作废** —— 原文写的是
 *    「注意它**仍然退 0**：清单还空着不是被检代码的失败，不该把整套 verify 弄红」，
 *    那条理由是**立骨架、`BLOCKS` 还空着**时期的。现在清单已满 12 块 119 条 ⇒
 *    清单**短于**预期只能意味着守卫**停止保护**了，那是失败，必须红。
 */
...
/*
 * ── 清单下限自检（2026-09-20 加）────────────────────────────────────────────
 * ⚠️ **清单短于预期 = 守卫停止保护 = 失败，不是提示。**（Task 8 报的 F5。）实测：把 `const BLOCKS = [...]`
 *    整体换成 `[]`，本守卫**改前是 exit 0**（只多打一行 ⚠️）⇒ `run-all.mjs` 会把它算成**通过**。
 *    即「清单被清空 / 被删条」这件事此前**没有任何闸能发现**。
 * ⚠️ 下面两个数是**常量**：**有意**增删块时必须**同步改它** —— 这正是想要的效果
 *    （逼登记，而不是静默地少验几条）。
 * ⚠️ 失败时**只打一行**（见上：收尾那一行必须是最后一行非空行，多打会把读数挤下去）。
 */
const EXPECT_BLOCKS = 12
const EXPECT_TOTAL = 119
if (BLOCKS.length !== EXPECT_BLOCKS || total !== EXPECT_TOTAL) {
  console.log(
    `\n✗ 清单只有 ${BLOCKS.length} 块 / ${total} 条，**短于**预期的 ${EXPECT_BLOCKS} 块 / ${EXPECT_TOTAL} 条 ⇒ 守卫**保护的代码变少了**（被清空或删条）—— 这是**失败**不是提示；确属有意增删块时，同步改本文件的 EXPECT_BLOCKS / EXPECT_TOTAL。`,
  )
  process.exit(1)
}
