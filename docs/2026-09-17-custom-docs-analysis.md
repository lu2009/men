...
用户已在浏览器里打过一遍，**确认无误**（分页、双联排版、「编辑生产单」弹窗）。
...
## 8d. 第 5 张：自定义合格标签族（ic=13，已完成）

施工图 `docs/custom-docs-recon/01-ql.md`。**QL 与底座的重合面比 PS 还小**：
可原样用的只有 4 处（`escapeHtml` / `waitForImages` / 两个存储函数），另 4 处要参数化，其余全须新写。

在 PS 已丢掉的那几块之外，QL **再多丢两块**：

1. **它根本没有分页** —— `ae()` 一次产出全部标签，`page-break-after:always` 交给浏览器；
   全文 **0 个 `await`**（已证）。底座整套 `paginate.ts` 对它完全无对象。
2. **class 只有 4 个**：`qlabel-root` / `qlabel` / `qfield` / `qfield-qr|qfield-text`。

### 几处「照抄就会错」的点

- **QL 不认 `<br>`** —— 整串转义，`备注:加急<br>加配:X` 会**逐字显示 `<br>`**。
  底座 `renderMultiline` 按 `<br>` 切行**在这里是错的**。
- **打印 iframe 是真实尺寸**（`width:{widthMm}mm;height:{heightMm}mm`），底座是 `0×0`。
- **`orientation` 在 QL 不是死字段**（`printSilent` 读它）；**`printRotate90`** 是四张里唯一的旋转分支。
- **4 个 localStorage 键**（多两个固定张数相关，存**裸字符串**）。
- 清洗多一条：**全字段不可见时整体回落默认**。

### ★ 三入口结案：一份实现就够

「自定义 / 平开 / 推拉合格标签」共用同一组件同一 `ic`，**组件内部完全无感**（CONFIRMED）
⇒ 新版**一个抽屉实例 + 一个 `entry: 'all'|'ping'|'diao'` prop**。

行过滤**落在组件层**（`{...ctx, lines: filterLabelLines(...)}`），**没动 `printPayloads.ts`** ——
`labelRows()` 同时服务 hiprint 的 `lable` 模板，改它会连带影响那条链路。

### 验收

| 脚本 | 结果 |
|---|---|
| `ql-csscheck` | **两个夹具都逐字节相等**（838 / 1108 字节） |
| `ql-htmlcheck` / `ql-logiccheck` | 45/45 · 161/161 |
| 其余四张共 10 个 | **全过**（零回归） |
| `verify/ql-e2e.mjs` | 三入口过滤在真实数据上验通（8 + 40 = 48，互斥） |

### 已知缺口

`labelRows('lable')` **不产 `storeAddress`** ⇒「编辑标签」弹窗里那张门店特判的「位置」列**恒为空**。
修它要动 `printPayloads.ts`，本次没动，已标 `TODO(未确认)`。
...
| 12 | 自定义收据单 | 独立 | 从零做 |
| 16 | 自定义玻璃合片单 | C | 试点 |
| 15 | 自定义生产单2 | C | **抽底座 + 参数化**（77% 复用） |
| 14 | 自定义生产单 | B | **另写一套**（底座几乎不共用） |
| 13 | 自定义合格标签族 | A | **另写一套** + 一组件三入口 |

**结论：只有 C 家族那两张真的同源。** 另三张与底座的重合面都很小 ——
这说明「先做一张当试点、再抽底座」那条路是对的：**在真正共用的地方共用、不共用的地方老实另写**。
底座里 `escapeHtml` / `waitForImages` / `createQrEncoder` / `storage` 那几件对五张都通用，是净赚的。
