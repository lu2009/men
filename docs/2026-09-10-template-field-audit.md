...
>
> 运行时已验（`左锁内开` 命中 / `双开内开·双开外开` → `双开左` 命中 / `双开内右·双开外右` → `双开右` 命中）。
> 顺带确认：**平开非双开的取图语义（`{公式}{开向}` 精确匹配）与原版一致，没有内开↔外开回退**
> —— 全篇 20 处 `外开` 字面量已逐处核过（14 处是 Q/J 两张开向图映射表，6 处在 B0 的双开判断里），
> 不存在把外开折到内开的逻辑。选 `左锁外开` 而出不来图，只可能是该公式确实没画过 `左锁外开` 这张图。
