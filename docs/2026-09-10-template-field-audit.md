...
>
> 3. **开孔图不区分内开/外开（用户实测更正）。** 用户在实际旧版上确认：玻璃订单的**外开确实能显示**，
>    且 **左锁内开 与 左锁外开 用同一张图**、右锁内开 与 右锁外开 同一张、内左/外左、内右/外右 同理。
>    代码层面原版只把 `formulaID + 开向` 交给 `param1=getimage`，**归一发生在服务端**
>    （前端见到的结果就是「按 左/右 + 锁位 命中，内开外开共用」）。新系统公式图按方向存，
>    故改为**先精确命中、未命中再退到内/外互换的兄弟方向**（`inOutSibling`）；两方向都有图时仍各用各的。
>
>     ⚠️ 本节上一版曾据「全篇 20 处 `外开` 字面量里没有内↔外替换」断言「原版也没有回退、选外开就是没图」
>     —— **那个结论是错的**：归一不在前端 chunk 里，所以在前端 chunk 里找不到证据不等于不存在。
>     前端 chunk 里找不到证据 ≠ 行为不存在。
>
> 运行时已验：`左锁内开`（精确）/ `左锁外开`（退到左锁内开）/ `右锁内开` / `右锁外开`（退到右锁内开）/
> `内左`（退到外左）全部命中；`双开内开·双开外开` → `双开左`、`双开内右·双开外右` → `双开右` 命中；
> 库里确实没有的方向（如 `外右`）仍为空 —— 与「先精确、再兄弟」的语义一致。
