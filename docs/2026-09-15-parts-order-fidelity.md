...
- ✅ `computeParts` 出口按 `_keyOrder` 排序（缺省退回现状，不改变未迁移数据的行为）
- ✅ `Formulas.vue` 保存时写 `extra._keyOrder = Object.keys(parts)`
- 🌟 **额外收获**：`_keyOrder` 是原版字段名 ⇒ **导入原版公式时其顺序直接可用**，无需另建映射

### 5.1 端到端验证（2026-09-15，真机 CDP）

两条公式各建一行、算料、读生产单预览单元格：

| 列 | 移门（公式 81 `ZZ推拉测试`，2轨2扇/左外/2000*900*100） | 平开（公式 80 `平开`，2000*900*100 + 亮窗总高 300） |
|---|---|---|
| 门扇 | `光企高 → 勾企高 → 上下方 → 玻璃高 → 玻璃宽` | `光企高 → 上下方 → 玻璃高 → 玻璃宽` |
| 外框 | `边封 → 上滑 → 下滑` ← **修前是 `上滑→下滑→边封`** | `门框高 → 门框宽` |
| 亮窗/扣板 | `F槽高 → F槽宽 → 扣板宽 → 扣板高` | `扣板高 → 扣板宽 → 扣板厚` |

与 `_keyOrder` 逐位对照：`无亮窗边封[0] → 2轨上滑[1] → 2轨下滑[2]` ✅；
`扣板宽[26] → 扣板高[27] → F槽高[29] → F槽宽[30]` ✅（亮窗组整体前置于扣板组，是旧版代码里
`[...lw, ...padGroup()]` 的**分组序**，非 parts 序）。

**另注（本轮验证顺带发现）**：门扇列的顺序由 `DS_KW` **关键词数组**决定（`光企→方→…→玻璃高→玻璃宽`），
**不受** `_keyOrder` 影响 —— 这是旧版的分组语义，两家一致；受 `_keyOrder` 影响的是**纯 filter 型**的列
（外框的轨道组、亮窗/扣板的两个子组、平开的 windows 列）。
