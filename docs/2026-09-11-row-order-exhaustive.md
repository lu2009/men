...
| **D11** | ⛔ **不属于 Hui（转出为独立功能）** | **2026-09-14 归属复核**：跨订单按客户分组确实存在（`Ci()` @367536），但它是 **Home chunk 里工具栏按钮「订单汇总」的处理器**，数据源 `gn` 来自**服务端批量拉单**（@403383），同页还有「批量打印」「订单修改」等按钮，且 `Ci()` 设 `ic.value=6`（出货清单，与同页的 `ic=4` 标签 / `ic=5` 收据单并列）。<br>⇒ **这是「订单管理页」的功能，不是汇算页的**。我们 `Hui.vue` 的按单行为**正确**，不应在此实现跨订单分组。<br>规则留档（供将来做订单管理页时用）：按 `客户\|\|"未知客户"` 分组；组内 1 条原样深拷贝、行缺 `date` 用订单 `date` 回填；组内多条则 `{client, total:Σ, deposit:Σ, balance: total-deposit, 门数:Σ, receipt: 各行拼接}`，行 `date` 用所属订单的 `date` 回填。<br>另：mode 5（收据单）的金额清零由开关 `Zn` 门控（`Qn()` @354821），非无条件 |

### 补充说明（2026-09-14 之后的相关改动）

- **`link_no`（行级单号）已删除**：原版「单号」列可逐行编辑并作为「序号优先」的排序键。
  删除后 `orderedLines` 的 `orderPrefix` 退化为**订单级单号**
  → 「序号优先」当前恒等于原序。若将来恢复行级单号，排序会随之恢复。
  > ⚠️ **2026-09-18 更正删列理由**：本条原先写「移植时丢了编辑器导致恒为 NULL」——
  > **那个推理是错的**。行级单号的**写点不在前端编辑器，在旧服务端**：
  > `ensureLineNumbers()`（`server/src/modules/order/line-number.service.ts:120-235`）会生成 `N-YY/MM/DD`
  > 并落库到明细行的 `单号`，调用点两处（`progress.service.ts:667`、`formula.service.ts:201`）。
  > 丢的只是**唯一的手动编辑入口**，不是数据本身 —— 由「编辑器丢了」推出「这列没用」不成立。
  > 详见 `docs/2026-09-18-order-no-semantics.md`。
