...
>
> ⚠️ **另外别被这张表里的 `parentsubsidiary`（全小写）带跑**：真 `formulaType` 用的是
> **驼峰 `parentSubsidiary`**（旧版 `:912-914`/`:2213`/`:2236`/`:2133` 全是驼峰，
> 我们的 `printPayloads.ts` 也按驼峰比）。2026-09-19 已把我们的取值对齐成驼峰并迁移数据
> （迁移 0024）—— 写成小写会让**子母门的算料单据静默走错分支**。
...
- **表格行操作**：材料名/数量/计算结果 onBlur 触发重算与反推 `v`；删除（Popconfirm）、复制（含「大小扇上下方及玻璃」/「复制成单玻」/「同时复制勾企高」等分支）。
  ...
  > ⚠️ **2026-09-19 更正：上面「数量」那一项不成立。**
  > 数量 onBlur（`_0x293164`，`Diao.deobfuscated.js:3283-3292`）**只写 `def.quantity`，
  > 不重算、不反推 `v`**。触发重算的是**材料名**（`_0x11d327`，`:1926-1928`）与
  > **计算结果**（`_0x5d0636`，`:3293-3300`）两条。
  > 另外「复制成单玻」这四个字**源码里没有** —— 实际是那句
  > `只能复制成<名>情况，是否继续?` 的确认框文案（`s = 行名.includes("单玻") ? 行名 : 行名+"单玻"`，
  > `:1978-1989`）。逐字证据见 `docs/2026-09-19-diao-audit.md` §2.4b。
