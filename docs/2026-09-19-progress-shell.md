...
2. **新版没有按 `P2` 控制显隐**：`P2 = (registrant === name) || name === '开门红'` 依赖
   ...
   所以按钮**恒渲染**（且**可点**，见上第 1 条）。要么补上 `P2` 等价条件、
   要么明确决定「任何人都能看到这颗按钮」。⚠️ `'开门红'` 那半条本来也不可达（见 §6.3），
   别顺手把它当笔误删掉。

   > ⚠️ **2026-09-19 更新**：当初悬空的「账号类型映射」**已定**（`-analysis.md` §10 那张表：
   > `role='scanner'` ≈ 旧版 `defaulted=2`，其余 `role` ≈ 普通 PC 账号；
   > 服务端白名单见 `docs/2026-08-21-auth-design.md`）。等于 `P2` 的两个概念现在都有了：
   > `registrant` ↔ 登录响应/`GET /auth/me` 的 `tenant.name`，`name` ↔ `user.name`。
   > **但这段行为本身没变**（按钮仍恒渲染）—— 要不要按 `P2` 收显隐是前端的事，尚未决定。
