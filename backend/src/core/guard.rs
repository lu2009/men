...
//!    例如 `/api/v1/progress` 放行，并不代表以后新加的 `/api/v1/progress/delete` 也放行
//!    —— 同前缀的新端点照样要 admin。⇒ **老模块里新加的端点也默认拦住**。
//!
...
//! ## 为什么放行的白名单里只有这些
//!
//! 扫码工的手机上只有扫码生产页（`/Qrscanner`）：读行数据 → 提交工序 → 打标签。
//! 旧版同一个页面对 `defaulted=2` 关掉的按钮（设置工序、扫码查单）这里同样不放行，
//! 见 `docs/2026-09-19-qrscanner-analysis.md` §6.3。
//!
...
mod tests {
    ...
    fn scanner_can_use_scan_surface() {
        for (m, p) in [
            ...
            (Method::GET, "/api/v1/progress"),
            (Method::GET, "/api/v1/progress/more"),
