...
use super::model::{FormulaRequest, ImageRequest};
use super::service;
...
pub async fn list_images(
    ...
    let items = service::list_images(&state.pool, user.tenant_id, id).await?;
    Ok(response::ok(serde_json::to_value(items).unwrap()))
}
...
pub async fn add_image(
    ...
    let item = service::add_image(&state.pool, user.tenant_id, id, req).await?;
    Ok(response::ok(serde_json::to_value(item).unwrap()))
}
...
pub async fn delete_image(
    ...
    service::delete_image(&state.pool, user.tenant_id, id, image_id).await?;
    Ok(response::ok(json!({ "deleted": true })))
}
